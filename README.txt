- install apache, php 5.6 or higher (test 11) 
...
- Then install confirm if you have the following php modules installed:
...
- change the correspondent apache security configurations, if active (/etc/modsecurity/modsecurity.conf) - (Only if you get request body limit exceed or something similar)
	#to 32MB
...
- change the following apache vhost configurations if apply (this is, inside of your virtual-host configuration add the following lines):
...
- then (if apply) for each project, add the cronjobs:
	* * * * * sudo -u www-data php /home/jplpinto/Desktop/phpframework/trunk/app/layer/presentation/<project_name>/webroot/script.php  --documentroot="/var/www/html/phpframework/trunk/app/" --url="http://<project_url>/module/workerpool/run_worker_pool_script" --urlpath="module/workerpool/run_worker_pool_script" --loglevel=3
