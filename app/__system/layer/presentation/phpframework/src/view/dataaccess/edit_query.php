 ...
 */
include $EVC->getUtilPath("WorkFlowUIHandler"); include $EVC->getUtilPath("BreadCrumbsUIHandler"); include $EVC->getUtilPath("TourGuideUIHandler"); $obj_data = isset($obj_data) ? $obj_data : null; $db_drivers = isset($db_drivers) ? $db_drivers : null; $selected_db_broker = isset($selected_db_broker) ? $selected_db_broker : null; $selected_db_driver = isset($selected_db_driver) ? $selected_db_driver : null; $selected_type = isset($selected_type) ? $selected_type : null; $selected_table = isset($selected_table) ? $selected_table : null; $selected_tables_name = isset($selected_tables_name) ? $selected_tables_name : null; $selected_table_attrs = isset($selected_table_attrs) ? $selected_table_attrs : null; $map_php_types = isset($map_php_types) ? $map_php_types : null; $map_db_types = isset($map_db_types) ? $map_db_types : null; $item_type = isset($item_type) ? $item_type : null; $is_hbn_obj_equal_to_file_name = isset($is_hbn_obj_equal_to_file_name) ? $is_hbn_obj_equal_to_file_name : null; $file_path = isset($file_path) ? $file_path : null; $obj = isset($obj) ? $obj : null; $WorkFlowTaskHandler = isset($WorkFlowTaskHandler) ? $WorkFlowTaskHandler : null; $rel_type = isset($rel_type) ? $rel_type : null; $is_covertable_sql = isset($is_covertable_sql) ? $is_covertable_sql : null; $name = isset($name) ? $name : null; $parameter_class = isset($parameter_class) ? $parameter_class : null; $parameter_map = isset($parameter_map) ? $parameter_map : null; $result_class = isset($result_class) ? $result_class : null; $result_map = isset($result_map) ? $result_map : null; $sql = isset($sql) ? $sql : null; $filter_by_layout_url_query = LayoutTypeProjectUIHandler::getFilterByLayoutURLQuery($filter_by_layout); $is_obj_valid = $obj_data || !$query_id; $head = ""; $main_content = ""; if ($is_obj_valid) { $get_layer_sub_files_url = $project_url_prefix . "admin/get_sub_files?bean_name=$bean_name&bean_file_name=$bean_file_name$filter_by_layout_url_query&path=#path#"; $manage_ai_action_url = $openai_encryption_key ? $project_url_prefix . "phpframework/ai/manage_ai_action?bean_name=$bean_name&bean_file_name=$bean_file_name" : null; $WorkFlowUIHandler = new WorkFlowUIHandler($WorkFlowTaskHandler, $project_url_prefix, $project_common_url_prefix, $external_libs_url_prefix, $user_global_variables_file_path, $webroot_cache_folder_path, $webroot_cache_folder_url); $WorkFlowQueryHandler = new WorkFlowQueryHandler($WorkFlowUIHandler, $project_url_prefix, $project_common_url_prefix, $db_drivers, $selected_db_broker, $selected_db_driver, $selected_type, $selected_table, $selected_tables_name, $selected_table_attrs, $map_php_types, $map_db_types); $head = $WorkFlowUIHandler->getHeader(); $head .= LayoutTypeProjectUIHandler::getHeader(); } $head .= '
...
<script language="javascript" type="text/javascript" src="' . $project_url_prefix . 'js/dataaccess/edit_single_query.js"></script>'; if ($is_obj_valid) { $head .= $WorkFlowQueryHandler->getHeader(); $head .= $WorkFlowQueryHandler->getDataAccessJavascript($bean_name, $bean_file_name, $path, $item_type, $hbn_obj_id, $get_layer_sub_files_url); $head .= '<script>
...
</div>'; $main_content .= TourGuideUIHandler::getHtml($entity, $project_url_prefix, $project_common_url_prefix, $online_tutorials_url_prefix); } else { $title = ($query_id ? "Edit" : "Add") . " Query"; $main_content = '
		<div class="top_bar' . ($popup ? " in_popup" : "") . '">
			<header>
				<div class="title" title="' . $path . '">' . $title . '</div>
			</header>
		</div>'; $main_content .= '<div class="error">Error: The system couldn\'t detect the selected object. Please refresh and try again...</div>'; } $main_content .= '</div>'; ?>
