...
</script>'; $back_icon = isset($_SERVER["HTTP_REFERER"]) && strpos($_SERVER["HTTP_REFERER"], "/db/manage_records?") !== false ? '<li class="back" data-title="Back Page"><a class="icon go_back" onClick="goBackPage(this)">Go Back</a></li>' : ''; $main_content = '
...
<div class="manage_records with_top_bar_section' . ($popup ? " in_popup" : "") . '">'; if (empty($table_exists)) $main_content .= '<div class="error">Table does not exist!</tr>'; else if (empty($table_fields)) $main_content .= '<div class="error">Table fields do not exist!</tr>'; else { $PaginationLayout->show_x_pages_at_once = 5; $data = $PaginationLayout->data; $data["style"] = "bootstrap1"; $pagination_html = $PaginationLayout->designWithStyle(1, $data); $head .= '<style>' . $PaginationLayout->getBootstrap1Css() . '</style>'; $main_content .= '<div class="conditions">
		<label>
			Conditions: 
		...
		<ul>'; if (!empty($conditions)) foreach ($conditions as $field_name => $field_value) $main_content .= getConditionHtml($field_name, $field_value, isset($conditions_operators[$field_name]) ? $conditions_operators[$field_name] : null); $main_content .= '	
		</ul>
	</div>'; $main_content .= '
			...
			<table class="display compact">
				<thead>
					<tr>
						...
						</th>'; foreach ($table_fields as $field_name => $field) { $label = ucwords(str_replace(array("_", "-"), " ", strtolower($field_name))); $sorts_field_name = isset($sorts[$field_name]) ? $sorts[$field_name] : null; $main_content .= '	<th class="table_header" attr_name="' . $field_name . '">
							' . $label . '
				...
				<tbody>'; if (empty($results)) $main_content .= '<tr><td class="empty" colspan="' . ($table_fields ? count($table_fields) + 2 + ($fks || $extra_fks ? 1 : 0) : 0) . '">Empty results...</tr>'; else { $t = count($results); for ($i = 0; $i < $t; $i++) $main_content .= getRowHtml($i, $table_fields, $table_fields_types, $pks, $fks, $extra_fks, $manage_records_url, $results[$i]); } $main_content .= '</tbody>
			</table>
		</div>
	...
	<li>
		...
		<td class="select_item">
			<input type="checkbox" name="selected_rows[]" value="' . $v43dd7d0051 . '" title="To remove this record, select this box and then click in the delete button below." />'; foreach ($pe2f18119 as $v4ef49e289c) $pf8ed4912 .= '<input type="hidden" name="selected_pks[' . $v43dd7d0051 . '][' . $v4ef49e289c . ']" value="' . (isset($pff59654a[$v4ef49e289c]) ? $pff59654a[$v4ef49e289c] : "") . '" />'; $pf8ed4912 .= '
		</td>'; foreach ($pca1f5780 as $v4ef49e289c => $v5d170b1de6) { $v90963767ae = isset($pff59654a[$v4ef49e289c]) ? $pff59654a[$v4ef49e289c] : null; $pe35b25f6 = false; $pb2253540 = isset($v7ff440f783[$v4ef49e289c]) ? $v7ff440f783[$v4ef49e289c] : null; if (TextValidator::isBinary($v90963767ae)) { $pe35b25f6 = true; $pfd5da3fb = new finfo(FILEINFO_MIME_TYPE); $v71dafe3739 = $pfd5da3fb->buffer($v90963767ae); if (MimeTypeHandler::isImageMimeType($v71dafe3739)) $v90963767ae = "<img src=\"data:$v71dafe3739;base64, " . base64_encode($v90963767ae) . "\" />"; else if (!MimeTypeHandler::isTextMimeType($v71dafe3739)) $v90963767ae = "<a onClick=\"downloadFile(this, '$v4ef49e289c')\">Download File</a>"; } if (!$pe35b25f6 && is_array($pb2253540) && !empty($pb2253540["options"]) && array_key_exists($v90963767ae, $pb2253540["options"])) $pf8ed4912 .= '<td attr_name="' . str_replace('"', "&quot;", $v4ef49e289c) . '" attr_value="' . str_replace('"', "&quot;", $v90963767ae) . '">' . (isset($pb2253540["options"][$v90963767ae]) ? $pb2253540["options"][$v90963767ae] : "") . '</td>'; else $pf8ed4912 .= '<td' . ($pe35b25f6 && $pb2253540 != "file" ? ' class="binary"' : '') . ' attr_name="' . str_replace('"', "&quot;", $v4ef49e289c) . '">' . ($pb2253540 == "file" ? '<div class="file_content">' . $v90963767ae . '</div>' : $v90963767ae) . '</td>'; } $pf8ed4912 .= '
		<td class="actions">
		...
		</td>'; if (($v571a648e93 || $v8949029d70) && $v63572f592a) { $pf8ed4912 .= '<td class="fks">'; if ($v571a648e93) foreach ($v571a648e93 as $pf71bb87b => $pd042c178) { $pc7b98f2b = ucwords(str_replace(array("_", "-"), " ", strtolower($pf71bb87b))); $pc7b98f2b = substr($pc7b98f2b, -1) == "y" ? substr($pc7b98f2b, 0, -1) . "ies" : $pc7b98f2b . "s"; $v1563ad01df = str_replace("#table#", $pf71bb87b, $v63572f592a); foreach ($pd042c178 as $pfe6103e2 => $v97915b9670) $v1563ad01df .= "&conditions[$pfe6103e2]=" . (isset($pff59654a[$v97915b9670]) ? $pff59654a[$v97915b9670] : ""); $pf8ed4912 .= '<a fk_table="' . $pf71bb87b . '" href="' . $v1563ad01df . '">' . $pc7b98f2b . '</a>'; } if ($v8949029d70) foreach ($v8949029d70 as $pf71bb87b => $pd042c178) { $pc7b98f2b = ucwords(str_replace(array("_", "-"), " ", strtolower($pf71bb87b))); $pc7b98f2b = substr($pc7b98f2b, -1) == "y" ? substr($pc7b98f2b, 0, -1) . "ies" : $pc7b98f2b . "s"; $pc7b98f2b .= $v571a648e93 && $v571a648e93[$pf71bb87b] ? " 2" : ""; $v1563ad01df = str_replace("#table#", $pf71bb87b, $v63572f592a); foreach ($pd042c178 as $pfe6103e2 => $v97915b9670) $v1563ad01df .= "&conditions[$pfe6103e2]=" . (isset($pff59654a[$v97915b9670]) ? $pff59654a[$v97915b9670] : ""); $pf8ed4912 .= '<a fk_table="' . $pf71bb87b . '" href="' . $v1563ad01df . '">' . $pc7b98f2b . '</a>'; } $pf8ed4912 .= '</td>'; } $pf8ed4912 .= '
	</tr>'; return $pf8ed4912; } ?>
