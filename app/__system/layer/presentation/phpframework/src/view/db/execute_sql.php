 ...
 */
$manage_ai_action_url = $openai_encryption_key ? $project_url_prefix . "phpframework/ai/manage_ai_action?bean_name=$bean_name&bean_file_name=$bean_file_name" : null; $head = '
<!-- Add ACE editor -->
...
<script>
	...
	<div class="sql_results' . ($popup ? ' in_popup' : '') . '">
		<table class="display compact">'; $fields = isset($results["fields"]) ? $results["fields"] : null; $rows = isset($results["result"]) ? $results["result"] : null; if ($fields) { $main_content .= '<thead><tr>'; $t = count($fields); for ($i = 0; $i < $t; $i++) { $name = isset($fields[$i]->name) ? $fields[$i]->name : null; $main_content .= '<th class="table_header">' . $name . '</th>'; } $main_content .= '</tr></thead>'; } $main_content .= '<tbody>'; if (!empty($_POST)) { if (!empty($exception_message)) { $main_content .= '<tr>
			<td class="message error">
				' . $exception_message . '
			</td>
		</tr>'; } else if (empty($is_select_sql)) { $message = !empty($results) ? "SQL executed successfully." : "SQL executed unsuccessfully."; $main_content .= '<tr>
