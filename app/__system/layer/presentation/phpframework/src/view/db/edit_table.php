...
</script>'; $show_sub_menu = empty($table && !$table_exists) && empty($action == "delete" && $e === true); $main_content = '
...
<div class="edit_table' . ($with_advanced_options ? " with_advanced_options" : "") . '">'; if ($table && !$table_exists) $main_content .= '<div class="error">Table does not exists!</div>'; else if ($action == "delete" && $e === true) $main_content .= '<div class="message">Table deleted successfully!</div>'; else if ($obj) { $allow_sort = $obj->allowTableAttributeSorting(); $main_content .= '
	...
	<div class="table_sql_statements">
		<form method="post">
			...
			<textarea class="hidden" name="data">' . json_encode($data) . '</textarea>
			'; if (!empty($sql_statements)) { foreach ($sql_statements as $idx => $sql) $main_content .= '<div class="sql_statement">
			...
			</div>'; } else $main_content .= '<div>' . (isset($status_message) ? $status_message : "") . '</div>		
			<div class="save_button">
				<input class="back" type="button" name="back" value="Back" onClick="return onBackButton(this, 0);" />
			</div>'; $main_content .= '
		</form>
	...
	<div class="table_errors">'; if (!empty($error_message)) $main_content .= '<div class="error">' . $error_message . (!empty($errors) ? '<br/>Please see errors bellow...' : '') . '</div>'; else $main_content .= '<div>SQL executed successfully!</div>'; if (!empty($errors)) $main_content .= '<div class="errors">
			<label>Errors:</label>
			<ul>
