 ...
 */
 include_once get_lib("org.phpframework.util.text.TextValidator"); include_once get_lib("org.phpframework.util.MimeTypeHandler"); $manage_record_action_url = $project_url_prefix . "phpframework/db/manage_record_action?layer_bean_folder_name=$layer_bean_folder_name&bean_name=$bean_name&bean_file_name=$bean_file_name&table=$table"; $head = '
<!-- Add MD5 JS File -->
...
<div class="manage_record' . ($popup ? ' in_popup' : '') . '">
	...
	</div>'; if (empty($table_exists)) $main_content .= '<div class="error">Table does not exist!</div>'; else if (empty($table_fields)) $main_content .= '<div class="error">Table fields do not exist!</tr>'; else if (empty($results) && $action != "insert") $main_content .= '<div class="error">Record does not exists!</div>'; else { foreach ($pks as $field_name) $main_content .= '<input type="hidden" name="' . $field_name . '" value="' . (isset($results[$field_name]) ? $results[$field_name] : "") . '" />'; $main_content .= '<table>'; foreach ($table_fields as $field_name => $field) { $field_value = isset($results[$field_name]) ? $results[$field_name] : null; $is_binary = false; $label = ucwords(str_replace(array("_", "-"), " ", strtolower($field_name))); $field_html_type = isset($table_fields_types[$field_name]) ? $table_fields_types[$field_name] : null; $field_html_type = $field_html_type ? $field_html_type : "text"; if (TextValidator::isBinary($field_value)) { $is_binary = true; $finfo = new finfo(FILEINFO_MIME_TYPE); $mime_type = $finfo->buffer($field_value); if (MimeTypeHandler::isImageMimeType($mime_type)) $field_value = "<img src=\"data:$mime_type;base64, " . base64_encode($field_value) . "\" />"; else if (!MimeTypeHandler::isTextMimeType($mime_type)) $field_value = "<a onClick=\"downloadFile(this, '$field_name')\">Download File</a>"; } $main_content .= '
				<tr>
					<th>' . $label . ': </th>
					<td>'; if ($is_binary && $field_html_type != "file") $main_content .= $field_value; else { $options = null; if (is_array($field_html_type)) { $options = isset($field_html_type["options"]) ? $field_html_type["options"] : null; $field_html_type = isset($field_html_type["type"]) ? $field_html_type["type"] : null; $field_html_type = $options ? $field_html_type : "text"; } if ($field_html_type == "textarea") $main_content .= '<textarea name="' . $field_name . '">' . $field_value . '</textarea>'; else if ($field_html_type == "file") { $main_content .= '<div class="file_content">' . $field_value . '</div>'; $main_content .= '<input type="file" name="' . $field_name . '" />'; } else if ($field_html_type == "select") { $main_content .= '<select name="' . $field_name . '">'; $value_exists = false; if (is_array($options)) foreach($options as $v => $l) { $main_content .= '<option value="' . $v . '"' . ($v == $field_value ? ' selected' : '') . '>' . $l . '</option>'; if ($v == $field_value) $value_exists = true; } if (!$value_exists) $main_content .= '<option value="' . $field_value . '" selected>' . $field_value . '</option>'; $main_content .= '</select>'; } else if ($field_html_type == "checkbox" || $field_html_type == "radio") $main_content .= '<input type="' . $field_html_type . '" name="' . $field_name . '" value="1"' . ($field_value == 1 ? ' checked' : '') . '/>'; else $main_content .= '<input type="' . $field_html_type . '" name="' . $field_name . '" value="' . $field_value . '"/>'; } $main_content .= '</td>
		</tr>'; } $main_content .= '</table>'; } $main_content .= '</div>'; ?>
