...
'; $head .= isset($sla_js_head) ? $sla_js_head : null; $head .= WorkFlowPresentationHandler::getPresentationBrokersHtml($presentation_brokers, $choose_bean_layer_files_from_file_manager_url, $get_file_properties_url, $upload_bean_layer_files_from_file_manager_url); $head .= WorkFlowPresentationHandler::getBusinessLogicBrokersHtml($business_logic_brokers, $choose_bean_layer_files_from_file_manager_url, $get_file_properties_url); $head .= WorkFlowPresentationHandler::getDataAccessBrokersHtml($data_access_brokers, $choose_bean_layer_files_from_file_manager_url); $head .= WorkFlowPresentationHandler::getDaoLibAndVendorBrokersHtml($choose_dao_files_from_file_manager_url, $choose_lib_files_from_file_manager_url, $choose_vendor_files_from_file_manager_url, $get_file_properties_url); $head .= '</script>'; $head .= CMSPresentationLayerUIHandler::getHeader($project_url_prefix, $project_common_url_prefix, $get_available_blocks_list_url, $get_block_params_url, $create_entity_code_url, $available_blocks_list, $regions_blocks_list, $block_params_values_list, $view_includes, $blocks_join_points, $view_params_values_list, $selected_project_id, false, $head); $head .= LayoutTypeProjectUIHandler::getHeader(); $query_string = isset($_SERVER["QUERY_STRING"]) ? preg_replace("/dont_save_cookie=([^&])*/", "", str_replace(array("&edit_view_type=advanced", "&edit_view_type=simple"), "", $_SERVER["QUERY_STRING"])) : ""; $pos = strpos($file_path, "/src/view/") + strlen("/src/view/"); $view_prefix = substr($file_path, 0, $pos); $view_code = substr($file_path, $pos); $main_content = '
	...
	</div>'; if (!empty($obj_data)) { $code = isset($obj_data["code"]) ? $obj_data["code"] : null; $doc_type_props = WorkFlowPresentationHandler::getHtmlTagProps($code, "!DOCTYPE"); $html_props = WorkFlowPresentationHandler::getHtmlTagProps($code, "html"); $head_props = WorkFlowPresentationHandler::getHtmlTagProps($code, "head", array("get_inline_code" => true)); $body_props = WorkFlowPresentationHandler::getHtmlTagProps($code, "body", array("get_inline_code" => true)); $code_exists = !empty(trim($code)); $non_standard_code = $code_exists && empty($html_props["inline_code"]) && empty($head_props["inline_code"]) && empty($body_props["inline_code"]); $is_code_valid = !$code_exists || !empty($html_props["inline_code"]) || !empty($html_props["html_attributes"]) || !empty($head_props["inline_code"]) || !empty($head_props["html_attributes"]) || !empty($body_props["inline_code"]) || !empty($body_props["html_attributes"]); if ($non_standard_code) $body_props["inline_code"] = $code; if (!$is_code_valid) { $msg = $non_standard_code ? "Note that this code is a non standard html code." : "Note that some of the main tags (html, head or body) have attributes or dynamic code inside."; $main_content .= '<script>
	...
	<div class="template_obj view_obj with_top_bar_tab inactive' . ($popup ? " in_popup" : "") . '">
		...
		<div class="code_editor_settings regions_blocks_includes_settings collapsed" id="code_editor_settings">
			...
			<div id="resource_settings" class="resource_settings">
				' . SequentialLogicalActivityUIHandler::getSLAHtml($EVC, $PEVC, $project_url_prefix, $project_common_url_prefix, $layout_ui_editor_user_widget_folders_path, $webroot_cache_folder_path, $webroot_cache_folder_url, $tasks_contents, $db_drivers, $presentation_projects, $WorkFlowUIHandler, array( "save_func" => "saveView", )) . '
			</div>
		</div>
