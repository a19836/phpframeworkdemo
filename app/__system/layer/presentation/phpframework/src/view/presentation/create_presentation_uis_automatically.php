 ...
 */
include $EVC->getUtilPath("WorkFlowPresentationHandler"); include $EVC->getUtilPath("WorkFlowUIHandler"); include $EVC->getUtilPath("BreadCrumbsUIHandler"); $folder_path = isset($folder_path) ? $folder_path : null; $P = isset($P) ? $P : null; $filter_by_layout_url_query = LayoutTypeProjectUIHandler::getFilterByLayoutURLQuery($filter_by_layout); $head = '
...
<script language="javascript" type="text/javascript" src="' . $project_url_prefix . 'js/presentation/create_presentation_uis_automatically.js"></script>'; $main_content = ""; if (!empty($_POST["step_3"])) { $main_content .= '
	<div class="statuses">
		<div class="top_bar">
			<header>
		...
		<table>
			<tr>
			...
			</tr>'; $exists_status_ok = false; if (!empty($statuses)) foreach ($statuses as $table_name => $table_statuses) { $table_alias = isset($selected_tables_alias[$table_name]) ? $selected_tables_alias[$table_name] : null; foreach ($table_statuses as $file_path => $status) { $status = ($status ? "ok" : "error"); $main_content .= '<tr>
				...
				</tr>'; if ($status) $exists_status_ok = true; } } $main_content .= '
		</table>
	</div>'; if ($exists_status_ok) { $main_content .= '<script>
		...
		</script>'; } } else if (!empty($_POST["step_2"])) { if (!empty($active_brokers) && !empty($selected_tables)) { $create_presentation_uis_files_automatically_url = $project_url_prefix . "phpframework/presentation/create_presentation_uis_files_automatically?bean_name=$bean_name&bean_file_name=$bean_file_name$filter_by_layout_url_query&path=$path&db_layer=$db_layer&db_layer_file=$db_layer_file&db_driver=$db_driver&include_db_driver=$include_db_driver&type=$type&authenticated_template=$authenticated_template&non_authenticated_template=$non_authenticated_template&overwrite=$overwrite&users_perms_folder=$users_perms_folder"; $get_tables_ui_props_url = $project_url_prefix . "phpframework/presentation/get_presentation_tables_ui_props_automatically?bean_name=$bean_name&bean_file_name=$bean_file_name$filter_by_layout_url_query&path=$path&db_layer=$db_layer&db_layer_file=$db_layer_file&db_driver=$db_driver&include_db_driver=$include_db_driver&type=$type"; if ($active_brokers) foreach ($active_brokers as $broker_name => $aux) $active_brokers[$broker_name] = 1; if ($active_brokers_folder) foreach ($active_brokers_folder as $broker_name => $broker_folder) if (empty($broker_folder)) unset($active_brokers_folder[$broker_name]); if ($selected_tables_alias) foreach ($selected_tables_alias as $table_name => $table_alias) if (empty($table_alias) ) unset($selected_tables_alias[$table_name]); $post_data = array( "ab" => $active_brokers, "abf" => $active_brokers_folder, "st" => $selected_tables, "sta" => $selected_tables_alias, ); $tables_ui_props = $selected_tables ? $UserAuthenticationHandler->getURLContent($get_tables_ui_props_url, $post_data) : ""; $tables_ui_props = json_decode($tables_ui_props, true); $WorkFlowUIHandler = new WorkFlowUIHandler($WorkFlowTaskHandler, $project_url_prefix, $project_common_url_prefix, $external_libs_url_prefix, $user_global_variables_file_path, $webroot_cache_folder_path, $webroot_cache_folder_url); $tasks_settings = $WorkFlowTaskHandler->getLoadedTasksSettings(); $WorkFlowUIHandler->setTasksOrderByTag($allowed_tasks); $tasks_head = $WorkFlowUIHandler->printTasksCSSAndJS(); $tasks_contents = $WorkFlowUIHandler->printTasksProperties(); $js_load_functions = array(); $js_submit_functions = array(); $js_complete_functions = array(); foreach ($tasks_settings as $group_id => $group_tasks) foreach ($group_tasks as $task_type => $task_settings) if (is_array($task_settings)) { $tag = isset($task_settings["tag"]) ? $task_settings["tag"] : null; $js_load_functions[$tag] = isset($task_settings["settings"]["callback"]["on_load_task_properties"]) ? $task_settings["settings"]["callback"]["on_load_task_properties"] : null; $js_submit_functions[$tag] = isset($task_settings["settings"]["callback"]["on_submit_task_properties"]) ? $task_settings["settings"]["callback"]["on_submit_task_properties"] : null; $js_complete_functions[$tag] = isset($task_settings["settings"]["callback"]["on_complete_task_properties"]) ? $task_settings["settings"]["callback"]["on_complete_task_properties"] : null; } $choose_bean_layer_files_from_file_manager_url = $project_url_prefix . "admin/get_sub_files?bean_name=#bean_name#&bean_file_name=#bean_file_name#$filter_by_layout_url_query&path=#path#"; $upload_bean_layer_files_from_file_manager_url = $project_url_prefix . "admin/upload_file?bean_name=#bean_name#&bean_file_name=#bean_file_name#$filter_by_layout_url_query&path=#path#"; $choose_dao_files_from_file_manager_url = $project_url_prefix . "admin/get_sub_files?item_type=dao&path=#path#"; $choose_lib_files_from_file_manager_url = $project_url_prefix . "admin/get_sub_files?item_type=lib&path=#path#"; $choose_vendor_files_from_file_manager_url = $project_url_prefix . "admin/get_sub_files?item_type=vendor&path=#path#"; $get_file_properties_url = $project_url_prefix . "phpframework/admin/get_file_properties?bean_name=#bean_name#&bean_file_name=#bean_file_name#&path=#path#&class_name=#class_name#&type=#type#"; $get_query_properties_url = $project_url_prefix . "phpframework/dataaccess/get_query_properties?bean_name=#bean_name#&bean_file_name=#bean_file_name#&db_driver=$db_driver&db_type=$type&path=#path#&query_type=#query_type#&query=#query#&obj=#obj#&relationship_type=#relationship_type#"; $get_business_logic_properties_url = $project_url_prefix . "phpframework/businesslogic/get_business_logic_properties?bean_name=#bean_name#&bean_file_name=#bean_file_name#&path=#path#&service=#service#"; $get_broker_db_drivers_url = $project_url_prefix . "phpframework/db/get_broker_db_drivers?bean_name=$bean_name&bean_file_name=$bean_file_name&broker=#broker#&item_type=presentation"; $edit_task_source_url = $project_url_prefix . "phpframework/admin/edit_task_source?bean_name=$bean_name&bean_file_name=$bean_file_name$filter_by_layout_url_query&path=$path"; $head = '
		...
		</script>'; $head .= WorkFlowPresentationHandler::getFileManagerTreePopupHeader($project_url_prefix); $head .= '<script>'; $head .= WorkFlowPresentationHandler::getPresentationBrokersHtml($presentation_brokers, $choose_bean_layer_files_from_file_manager_url, $get_file_properties_url, $upload_bean_layer_files_from_file_manager_url); $head .= WorkFlowPresentationHandler::getBusinessLogicBrokersHtml($business_logic_brokers, $choose_bean_layer_files_from_file_manager_url, $get_file_properties_url); $head .= WorkFlowPresentationHandler::getDaoLibAndVendorBrokersHtml($choose_dao_files_from_file_manager_url, $choose_lib_files_from_file_manager_url, $choose_vendor_files_from_file_manager_url, $get_file_properties_url); $head .= WorkFlowPresentationHandler::getDataAccessBrokersHtml($data_access_brokers, $choose_bean_layer_files_from_file_manager_url); $head .= '$(function () {
			onLoadTableSettings();
		...
		</div>'; } if (empty($selected_tables)) $main_content .= '<div class="main_error">No tables selected. Please go back and select at least one table...</div>'; else if (empty($active_brokers)) $main_content .= '<div class="main_error">No brokers active. Please go back and select at least one broker...</div>'; } else if (!empty($_POST["step_1"])) { $head .= '<script>
		...
		<form method="post">
			...
			<div class="brokers">
				...
				<table>
					...
					</tr>'; foreach ($brokers as $broker_name => $broker) if (is_a($broker, "IBusinessLogicBrokerClient") || is_a($broker, "IDataAccessBrokerClient") || is_a($broker, "IDBBrokerClient")) { $broker_type_label = is_a($broker, "IBusinessLogicBrokerClient") ? "Business Logic Broker" : ( is_a($broker, "IHibernateDataAccessBrokerClient") ? "Hibernate Data Access Broker" : ( is_a($broker, "IIbatisDataAccessBrokerClient") ? "Ibatis Data Access Broker" : ( is_a($broker, "IDBBrokerClient") ? "DB Broker" : "" ) ) ); $main_content .= '
					<tr>
			...
			<div class="users_permissions">
				...
				<div class="users_management_admin_panel_popup myfancypopup">
					<iframe></iframe>
				</div>'; if (!empty($available_user_types) && !empty($available_activities)) { $main_content .= '
				<table>
					<tr>
						<th class="table_header status">User Type</th>'; foreach ($available_activities as $activity_id => $activity_name) $main_content .= '<th class="table_header ' . $activity_name . '">' . ucwords($activity_name) . '</th>'; $main_content .= '</tr>'; foreach ($available_user_types as $user_type_id => $user_type_name) { $main_content .= '<tr>
						<td class="name">' . $user_type_name . '</td>'; foreach ($available_activities as $activity_id => $activity_name) $main_content .= '<td class="' . $activity_name . '"><input type="checkbox" name="users_perms[' . $user_type_id . '][' . $activity_id . ']" value="1" /></td>'; $main_content .= '</tr>'; } $main_content .= '</table>
			...
			<div class="list_and_edit_users">
				...
				<div class="info">
					<select name="list_and_edit_users[]" multiple>
						<option value="" disabled>Choose at least one user to enable this feature...</option>'; if ($available_user_types) foreach ($available_user_types as $user_type_id => $user_type_name) $main_content .= '<option value="' . $user_type_id . '" ' . ($user_type_id != UserUtil::PUBLIC_USER_TYPE_ID ? 'selected' : '') . '>' . $user_type_name . '</option>'; $main_content .= '
						</select>
					</div>
				</div>
				...
				<div class="non_authenticated_template">
					...
					<select name="non_authenticated_template">
						<option value="">-- DEFAULT --</option>'; $t = count($available_templates); for ($i = 0; $i < $t; $i++) $main_content .= '<option>' . $available_templates[$i] . '</option>'; $main_content .= '	
					</select>
		...
		<form method="post">'; if (empty($path)) $main_content .= '<div class="error">You cannot execute this action with an undefined path.</div>'; else if (empty($PEVC)) $main_content .= '<div class="error">Bean name doesn\'t exist. If this problem persists, please talk with the sys-admin.</div>'; else if (!is_dir($folder_path)) $main_content .= '<div class="error">You can only execute this action inside of a folder.</div>'; else if (!empty($db_drivers)) { $main_content .= '
			...
			<div class="db_driver">
				...
				<select name="db_driver" onChange="onChangeDBDriver(this)">'; if ($db_drivers) foreach ($db_drivers as $db_driver_name => $db_driver_props) { $db_driver_bean_file_name = isset($db_driver_props[1]) ? $db_driver_props[1] : null; $db_driver_bean_name = isset($db_driver_props[2]) ? $db_driver_props[2] : null; $main_content .= '<option bean_file_name="' . $db_driver_bean_file_name . '" bean_name="' . $db_driver_bean_name . '" value="' . $db_driver_name . '"' . ($selected_db_driver == $db_driver_name ? " selected" : "") . '>' . $db_driver_name . ($db_driver_props ? '' : ' (Rest)') . '</option>'; } $main_content .= '	
				</select>
				<input type="hidden" name="db_layer" value="' . $db_layer . '" />
				<input type="hidden" name="db_layer_file" value="' . $db_layer_file . '" />
			</div>
			...
			</div>'; } else $main_content .= '<div class="error">There are no DB Layers.<br/>Apparently you have your Presentation Layer without any Business Logic, Data Access or DB Layers, which means your application is not correctly configured.<br/>Please go to "Layers Management" Menu and configure correclty your Presentation Layer.</div>'; $main_content .= '
		</form>
	</div>'; } ?>
