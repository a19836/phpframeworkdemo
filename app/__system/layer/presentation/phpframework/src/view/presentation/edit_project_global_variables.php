...
<div class="global_vars_obj with_top_bar_tab">
	...
	<div id="form_global_vars">
		...
		<table class="vars">
			...
			</tr>'; if (isset($vars) && is_array($vars)) foreach ($vars as $name => $value) $main_content .= getGlobalVarHtml($name, $value, !in_array($name, $reserved_vars)); $main_content .= '
		</table>
	</div>
</div>
...
<div class="hide_show_header minimize" onClick="toggleHeader(this)" title="Minimize/Maximize"></div>'; function getGlobalVarHtml($v1cfba8c105, $pa6209df1, $v074bed71f7 = true) { $pf8ed4912 = '
	<tr>
		...
		<td class="var_value">'; if (is_array($pa6209df1)) { $pf72c1d58 = isset($pa6209df1["items"]) ? $pa6209df1["items"] : null; $v67db1bd535 = isset($pa6209df1["value"]) ? $pa6209df1["value"] : null; $pcda0061d = isset($pa6209df1["force_raw_keys"]) ? $pa6209df1["force_raw_keys"] : null; $v7959970a41 = false; $pf8ed4912 .= '<select class="var_value" name="vars_value[]" allownull="true" onChange="onChangeSimpleFormGlobalVarSelect(this)">'; $v78b74a1eb5 = ''; if (is_array($pf72c1d58)) { $v651d593e1f = $pcda0061d || array_keys($pf72c1d58) !== range(0, count($pf72c1d58) - 1); foreach ($pf72c1d58 as $pe5c5e2fe => $v956913c90f) { $pb69caaf8 = isset($v67db1bd535) && ( ($v651d593e1f && "$pe5c5e2fe" == "$v67db1bd535") || (!$v651d593e1f && "$v956913c90f" == "$v67db1bd535") ); $v78b74a1eb5 .= '<option value="' . ($v651d593e1f ? $pe5c5e2fe : $v956913c90f) . '"' . ($pb69caaf8 ? ' selected' : '') . '>' . $v956913c90f . '</option>'; if ($pb69caaf8) $v7959970a41 = true; } } $pf8ed4912 .= '<option value="__DEFAULT__"' . (!$v7959970a41 && $v67db1bd535 == "__DEFAULT__" ? ' selected' : '') . '>-- default --</option>'; $pf8ed4912 .= $v78b74a1eb5; if (!$v7959970a41 && strlen($v67db1bd535) && $v67db1bd535 != "__DEFAULT__") $pf8ed4912 .= '<option selected>' . $v67db1bd535 . '</option>'; $pf8ed4912 .= '</select>'; } else { if ($pa6209df1 === true) $pa6209df1 = "true"; else if ($pa6209df1 === false) $pa6209df1 = "false"; else if ($pa6209df1 === null) $pa6209df1 = "null"; $pf8ed4912 .= '<input type="text" class="var_value" name="vars_value[]" value="' . str_replace('"', '&quot;', $pa6209df1) . '" allownull="true" onBlur="onBlurSimpleFormGlobalVarInput(this)" />'; } $pf8ed4912 .= '</td>
		<td class="buttons">
