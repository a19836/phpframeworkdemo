...
'; $head .= WorkFlowPresentationHandler::getPresentationBrokersHtml($presentation_brokers, $choose_bean_layer_files_from_file_manager_url, $get_file_properties_url, $upload_bean_layer_files_from_file_manager_url); $head .= WorkFlowPresentationHandler::getDaoLibAndVendorBrokersHtml($choose_dao_files_from_file_manager_url, $choose_lib_files_from_file_manager_url, $choose_vendor_files_from_file_manager_url, $get_file_properties_url); $head .= '</script>'; $head .= CMSPresentationLayerJoinPointsUIHandler::getHeader(); $head .= LayoutTypeProjectUIHandler::getHeader(); $main_content = '
	...
	</div>'; if (!empty($module)) { if (empty($module["enabled"])) $main_content .='<div class="invalid">Warning: This module is currently DISABLED!</div>'; if (!empty($hard_coded)) $main_content .='<div class="invalid">Alert: The system detected that the block id is different than the current file name. We advise you to edit this file with the Advanced UI, otherwise you may overwrite other people\'s changes...</div>'; $main_content .= WorkFlowPresentationHandler::getChooseFromFileManagerPopupHtml($bean_name, $bean_file_name, $choose_bean_layer_files_from_file_manager_url, $choose_dao_files_from_file_manager_url, $choose_lib_files_from_file_manager_url, $choose_vendor_files_from_file_manager_url, null, null, null, null, null, $presentation_brokers); $image = '<span class="no_photo">No Photo</span>'; if (!empty($module["images"][0]["url"])) { if (preg_match("/\.svg$/i", $module["images"][0]["url"]) && !empty($module["images"][0]["path"]) && file_exists($module["images"][0]["path"])) $image = file_get_contents($module["images"][0]["path"]); else $image = '<img src="' . $module["images"][0]["url"] . '" />'; } $main_content .= '
	<div class="block_obj' . (!$popup ? " with_top_bar_section" : "") . '">
		...
		</div>'; if (!empty($module["settings_html"])) $main_content .= '
			<div class="module_settings">
				<label>Module\'s Settings:</label>
				<div class="settings">
					' . (isset($module["settings_html"]) ? $module["settings_html"] : "") . '
				</div>
			</div>'; $main_content .= CMSPresentationLayerJoinPointsUIHandler::getBlockJoinPointsJavascriptObjs($block_join_points, $block_local_join_points); $main_content .= CMSPresentationLayerJoinPointsUIHandler::getBlockJoinPointsHtml(isset($module["join_points"]) ? $module["join_points"] : null, $block_id, empty($obj_data["code"]), isset($module["module_handler_impl_file_path"]) ? $module["module_handler_impl_file_path"] : null); $main_content .= '
		<script>
