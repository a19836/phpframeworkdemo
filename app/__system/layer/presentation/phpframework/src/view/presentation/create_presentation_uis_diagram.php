 ...
 */
include $EVC->getUtilPath("CMSPresentationLayerUIHandler"); include $EVC->getUtilPath("BreadCrumbsUIHandler"); $filter_by_layout_url_query = LayoutTypeProjectUIHandler::getFilterByLayoutURLQuery($filter_by_layout); $head = ""; $main_content = ""; if (!empty($WorkFlowTaskHandler)) { $WorkFlowUIHandler = new WorkFlowUIHandler($WorkFlowTaskHandler, $project_url_prefix, $project_common_url_prefix, $external_libs_url_prefix, $user_global_variables_file_path, $webroot_cache_folder_path, $webroot_cache_folder_url); $BrokersWorkFlowUIHandler = new WorkFlowUIHandler($BrokersWorkFlowTaskHandler, $project_url_prefix, $project_common_url_prefix, $external_libs_url_prefix, $user_global_variables_file_path, $webroot_cache_folder_path, $webroot_cache_folder_url); $tasks_settings = $BrokersWorkFlowTaskHandler->getLoadedTasksSettings(); $BrokersWorkFlowUIHandler->setTasksOrderByTag($allowed_tasks); $tasks_head = $BrokersWorkFlowUIHandler->printTasksCSSAndJS(); $tasks_contents = $BrokersWorkFlowUIHandler->printTasksProperties(); $js_load_functions = array(); $js_submit_functions = array(); $js_complete_functions = array(); foreach ($tasks_settings as $group_id => $group_tasks) foreach ($group_tasks as $task_type => $task_settings) if (is_array($task_settings)) { $tag = isset($task_settings["tag"]) ? $task_settings["tag"] : null; $js_load_functions[$tag] = isset($task_settings["settings"]["callback"]["on_load_task_properties"]) ? $task_settings["settings"]["callback"]["on_load_task_properties"] : null; $js_submit_functions[$tag] = isset($task_settings["settings"]["callback"]["on_submit_task_properties"]) ? $task_settings["settings"]["callback"]["on_submit_task_properties"] : null; $js_complete_functions[$tag] = isset($task_settings["settings"]["callback"]["on_complete_task_properties"]) ? $task_settings["settings"]["callback"]["on_complete_task_properties"] : null; } $choose_dao_files_from_file_manager_url = $project_url_prefix . "admin/get_sub_files?item_type=dao&path=#path#"; $choose_lib_files_from_file_manager_url = $project_url_prefix . "admin/get_sub_files?item_type=lib&path=#path#"; $choose_vendor_files_from_file_manager_url = $project_url_prefix . "admin/get_sub_files?item_type=vendor&path=#path#"; $get_broker_db_data_url = $project_url_prefix . "phpframework/dataaccess/get_broker_db_data?bean_name=$bean_name&bean_file_name=$bean_file_name"; $get_file_properties_url = $project_url_prefix . "phpframework/admin/get_file_properties?bean_name=#bean_name#&bean_file_name=#bean_file_name#&path=#path#&class_name=#class_name#&type=#type#"; $choose_bean_layer_files_from_file_manager_url = $project_url_prefix . "admin/get_sub_files?bean_name=#bean_name#&bean_file_name=#bean_file_name#$filter_by_layout_url_query&path=#path#"; $upload_bean_layer_files_from_file_manager_url = $project_url_prefix . "admin/upload_file?bean_name=#bean_name#&bean_file_name=#bean_file_name#$filter_by_layout_url_query&path=#path#"; $get_tables_ui_props_url = $project_url_prefix . "phpframework/presentation/get_presentation_tables_ui_props_automatically?bean_name=$bean_name&bean_file_name=$bean_file_name$filter_by_layout_url_query&path=$path&db_driver=#db_driver#&type=#type#"; $get_query_properties_url = $project_url_prefix . "phpframework/dataaccess/get_query_properties?bean_name=#bean_name#&bean_file_name=#bean_file_name#&db_driver=#db_driver#&db_type=#db_type#&path=#path#&query_type=#query_type#&query=#query#&obj=#obj#&relationship_type=#relationship_type#"; $get_business_logic_properties_url = $project_url_prefix . "phpframework/businesslogic/get_business_logic_properties?bean_name=#bean_name#&bean_file_name=#bean_file_name#&path=#path#&service=#service#"; $get_broker_db_drivers_url = $project_url_prefix . "phpframework/db/get_broker_db_drivers?bean_name=$bean_name&bean_file_name=$bean_file_name&broker=#broker#&item_type=presentation"; $users_management_admin_panel_url = $project_url_prefix . "phpframework/module/user/admin/index?bean_name=$bean_name&bean_file_name=$bean_file_name$filter_by_layout_url_query&path=$path&popup=1"; $get_entity_settings_url = $project_url_prefix . "phpframework/presentation/get_entity_settings?bean_name=$bean_name&bean_file_name=$bean_file_name&path=$path#entity#.php"; $edit_entity_admin_panel_url = $project_url_prefix . "phpframework/presentation/edit_entity?bean_name=$bean_name&bean_file_name=$bean_file_name&path=$path#entity#.php"; $are_entities_hard_coded_url = $project_url_prefix . "phpframework/presentation/are_entities_hard_coded?bean_name=$bean_name&bean_file_name=$bean_file_name&path=$path&entities=#entities#"; $get_current_path_sub_files_url = $project_url_prefix . "admin/get_sub_files?bean_name=$bean_name&bean_file_name=$bean_file_name$filter_by_layout_url_query&path=$path"; $edit_task_source_url = $project_url_prefix . "phpframework/admin/edit_task_source?bean_name=$bean_name&bean_file_name=$bean_file_name$filter_by_layout_url_query&path=$path"; $head = $WorkFlowUIHandler->getHeader(array("tasks_css_and_js" => false)); $head .= WorkFlowPresentationHandler::getFileManagerTreePopupHeader($project_url_prefix); $head .= LayoutTypeProjectUIHandler::getHeader(); $head .= '
	<!-- Add MD5 JS File -->
	...
	<div class="top_bar">
		...
		<div id="create_uis_files" class="myfancypopup with_title create_uis_files">
			...
			<div class="step step_1">
				...
				<div class="list_and_edit_users">
					...
					<div class="info">
						<select multiple>
							<option value="" disabled>Choose at least one user to enable this feature...</option>'; if (!empty($available_user_types)) foreach ($available_user_types as $user_type_id => $user_type_name) $main_content .= '<option value="' . $user_type_id . '" ' . ($user_type_id != UserUtil::PUBLIC_USER_TYPE_ID ? 'selected' : '') . '>' . $user_type_name . '</option>'; $main_content .= '
						</select>
					</div>
				</div>
		...
		</div>'; $menus = array( "Flush Cache" => array( "class" => "flush_cache", "html" => '<a onClick="return flushCache();"><i class="icon flush_cache"></i> Flush Cache</a>', ), "Empty Diagram" => array( "class" => "empty_diagram", "html" => '<a onClick="emptyDiagam();return false;"><i class="icon empty_diagram"></i> Empty Diagram</a>', ), 0 => array( "class" => "separator", "title" => " ", "html" => " ", ), "Zoom In" => array( "class" => "zoom_in", "html" => '<a onClick="zoomInDiagram(this);return false;"><i class="icon zoom_in"></i> Zoom In</a>', ), "Zoom Out" => array( "class" => "zoom_out", "html" => '<a onClick="zoomOutDiagram(this);return false;"><i class="icon zoom_out"></i> Zoom Out</a>', ), "Zoom" => array( "class" => "zoom", "html" => '
			<a onClick="zoomEventPropagationDiagram(this);return false;"><i class="icon zoom"></i> <input type="range" min="0.5" max="1.5" step=".02" value="1" onInput="zoomDiagram(this);return false;" /> <span>100%</span></a>', ), "Zoom Reset" => array( "class" => "zoom_reset", "html" => '<a onClick="zoomResetDiagram(this);return false;"><i class="icon zoom_reset"></i> Zoom Reset</a>', ), 1 => array( "class" => "separator", "title" => " ", "html" => " ", ), "Create UI Files" => array( "class" => "create_ui_files", "html" => '<a onClick="createUIFiles();return false;"><i class="icon create_ui_files"></i> Create UI Files</a>', ), "Load New Files" => array( "class" => "load_new_existent_files", "html" => '<a onClick="loadNewExistentFiles();return false;"><i class="icon load_new_existent_files"></i> Load New Files</a>', ), 2 => array( "class" => "separator", "title" => " ", "html" => " ", ), "Maximize/Minimize Editor Screen" => array( "class" => "tasks_flow_full_screen", "html" => '<a onClick="toggleFullScreen(this);return false;"><i class="icon full_screen"></i> Maximize Editor Screen</a>', ), 3 => array( "class" => "separator", "title" => " ", "html" => " ", ), "Save" => array( "class" => "save", "html" => '<a onClick="saveUIsDiagramFlow();return false;"><i class="icon save"></i> Save</a>', ), ); $WorkFlowUIHandler->setMenus($menus); $main_content .= $WorkFlowUIHandler->getContent(); $main_content .= '
	...
	</div>'; $main_content .= '</div>'; } else $error_message = "Error: Persentation UIs Bean undefined or folder path does not exist!"; ?>
