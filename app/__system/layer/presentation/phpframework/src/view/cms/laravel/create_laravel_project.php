...
'; $head .= LayoutTypeProjectUIHandler::getHeader(); $main_content = ""; if (!empty($_POST["step_1"])) { $msg = (!empty($status) ? 'Project created successfully!' : 'Error creating laravel project.') . (!empty($error_message) ? "<br/>$error_message" : ""); $main_content .= '<div class="status">
		<div class="top_bar">
			<header>
	...
	</div>'; if (!empty($status)) $main_content .= '<script>
...
var laravel_kit_stack_features = ' . (isset($laravel_kit_stack_features) ? json_encode($laravel_kit_stack_features) : "null") . ';
	...
	<div class="laravel_form">
		...
		<form method="post" onSubmit="return MyJSLib.FormHandler.formCheck(this);">
			...
			<div class="project_kit">
				...
				<select name="project_kit" onChange="onProjectKitChange(this)">'; if (!empty($laravel_kits)) foreach ($laravel_kits as $k => $v) $main_content .= '<option value="' . $k . '">' . $v . '</option>'; $main_content .= '
				</select>
			</div>
			<div class="project_testing_framework">
				<label>Testing Framework: </label>
				<select name="project_testing_framework">'; if (!empty($laravel_testing_frameworks)) foreach ($laravel_testing_frameworks as $k => $v) $main_content .= '<option value="' . $k . '">' . $v . '</option>'; $main_content .= '
				</select>
			</div>
			<div class="project_database">
				<label>Default Database Driver: </label>
				<select name="project_database">
					<!--option value="">-- none --</option-->'; if (!empty($laravel_databases)) foreach ($laravel_databases as $k => $v) $main_content .= '<option value="' . $k . '">' . $v . '</option>'; $main_content .= '
				</select>
			</div>
			<div class="project_db_driver">
				...
				<select name="project_db_driver" onChange="onChangeProjectWithDB(this)">
					...
					<option value="" disabled></option>'; if (!empty($db_drivers_names)) { $main_content .= '<optgroup label="Existent DBs">'; foreach ($db_drivers_names as $db_driver_name => $aux) $main_content .= '<option value="' . $db_driver_name . '">' . $db_driver_name . '</option>'; $main_content .= '</optgroup>'; } $main_content .= '
				</select>
			</div>
			<div class="new_db">
				<div class="db_host">
	...
	</div>'; } if (empty($is_shell_cmd_allowed)) $main_content .= '<div class="warning">PHP "' . ShellCmdHandler::FUNCTION_NAME . '" function is disabled. please enable this function to proceed.</div>'; else if (empty($laravel_bin_path)) $main_content .= '<div class="warning">Laravel bin path is undefined. Please be sure that laravel is installed globally and your web server has access to it.<br/>The current web server path is: "' . $apache_bin_path . '".</div>'; else if (empty($laravel_cmd_is_installed)) $main_content .= '<div class="warning">Laravel command is not installed. Please install laravel command line to proceed.</div>'; ?>
