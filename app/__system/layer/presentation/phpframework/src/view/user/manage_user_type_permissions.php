...
<div id="content">
	...
	<form method="post" onSubmit="return saveUserTypePermissions();">
		<div class="user_type">
			...
			<select name="user_type_id" onChange="updateUserTypePermissions(this)">'; foreach ($user_types as $name => $id) { $main_content .= '<option value="' . $id . '"' . (isset($user_type_id) && $user_type_id == $id ? ' selected' : '') . '>' . $name . '</option>'; } $main_content .= '	</select>
		</div>
			...
			<div id="pages_permissions_list">
				<table object_type_id="' . $page_object_type_id . '">
					<tr>
						...
						<td class="object_id">' . $page . '</td>'; foreach ($permissions as $permission_name => $permission_id) { $main_content .= '	<td class="user_type_permission user_type_permission_' . $permission_id . '" permission_id="' . $permission_id . '">'; if (!empty($available_page_permissions[$permission_name])) { $main_content .= '<input type="checkbox" name="permissions_by_objects[' . $page_object_type_id . '][' . $page . '][]" value="' . $permission_id . '" />'; } $main_content .= '	</td>'; } $main_content .= '	</tr>'; } $access_permission = isset($permissions["access"]) ? $permissions["access"] : null; $main_content .= '	</table>
			</div>
			...
			<div id="layers_permissions_list">
				<table class="mytree" object_type_id="' . $layer_object_type_id . '">
					...
					<tr>
						...
						<td></td>
					</tr>'; if ($layer_type) foreach ($layer_type as $layer_name => $layer) { $layer_props = isset($layers_props[$layer_type_name][$layer_name]) ? $layers_props[$layer_type_name][$layer_name] : null; if ($layer_type_name == "vendors" || $layer_type_name == "others") $object_id = $layer_name; else $object_id = "$layer_object_id_prefix/" . (isset($layers_object_id[$layer_type_name][$layer_name]) ? $layers_object_id[$layer_type_name][$layer_name] : ""); $main_content .= '
					<tr>
					...
					</tr>'; foreach ($layer as $folder_name => $folder) { $object_id = "$layer_object_id_prefix/" . (isset($layers_object_id[$layer_type_name][$layer_name]) ? $layers_object_id[$layer_type_name][$layer_name] : "") . "/$folder_name"; $indentation = str_repeat("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;", count(explode("/", $folder_name))); $icon_class = $layer_type_name == "db_layers" ? "db_driver" : "project"; $main_content .= '
					<tr>
