...
<div class="framework_update">
	...
	</div>'; if ($is_remote_update_allowed) { $main_content .= '
	<form method="post">
		<input type="hidden" name="step" value="' . ($step + 1) . '" />'; if ($step == 2 || ($step == 1 && empty($changed_files))) { $output = !empty($output) ? implode("\n", $output) : "No output detected.<br/>Please talk with your sysadmin to be sure everything runned as planned."; $main_content .= '<div>Update finished!</div>
		<pre class="code">' . $output . '</pre>
		<div>To refresh the framework please click <a href="javascript:void(0)" onClick="window.parent.document.location=window.parent.document.location;">here</a></div>'; } else if ($step == 1 && !empty($changed_files)) { $changed_authdb = false; foreach ($changed_files as $file) if (preg_match("/^other\/authdb\/\w+\.tbl$/", $file)) { $changed_authdb = true; break; } $changed_authdb = false; $main_content .= '
			...
			<input type="submit" name="update" value="Continue updating"/>'; } else $main_content .= '
			...
			<div>'; $main_content .= '</form>'; } else { $cms_path_str = preg_match("/\s/", CMS_PATH) ? '"' . CMS_PATH . '"' : CMS_PATH; $main_content .= '<div><strong>Update NOT possible, due to permission issues.</strong><br/>Please contact your system administrator to run the following command on your server as root user:</div>
	<pre class="command">sudo /bin/bash ' . $cms_path_str . 'other/script/update_git_repo.sh ' . $cms_path_str . '</pre>
	<div>If you want to keep your changes and merge them with the new ones, contact your system administrator to perform the update manually.</div>'; } $main_content .= '</div>'; ?>
