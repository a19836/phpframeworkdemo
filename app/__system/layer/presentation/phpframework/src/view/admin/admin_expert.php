...
if ($action == "edit") { include $EVC->getViewPath("admin/edit_raw_file"); } else if ($action == "upload") { $upload_url = "?admin_type=expert&path=$path&action=upload"; include $EVC->getViewPath("admin/upload_file"); } else if ($action == "get_sub_files") { include $EVC->getViewPath("admin/get_sub_files"); } else { include_once $EVC->getUtilPath("AdminMenuUIHandler"); include_once $EVC->getUtilPath("TourGuideUIHandler"); include_once $EVC->getUtilPath("HeatMapHandler"); $logged_name = $UserAuthenticationHandler->auth["user_data"]["name"] ? $UserAuthenticationHandler->auth["user_data"]["name"] : $UserAuthenticationHandler->auth["user_data"]["username"]; $logged_name_initials = explode(" ", $logged_name); $logged_name_initials = strtoupper(substr($logged_name_initials[0], 0, 1) . (isset($logged_name_initials[1]) ? substr($logged_name_initials[1], 0, 1) : "")); $notifications_url = $project_url_prefix . "admin/get_notifications"; $main_layers_properties = getMainLayersProperties(); $head = AdminMenuUIHandler::getHeader($project_url_prefix, $project_common_url_prefix); $head .= '
	...
	'; $head .= HeatMapHandler::getHtml($project_url_prefix); $main_content = getcontextMenusHtml(); $main_content .= '
	<div id="top_panel">
		<ul class="left">
	...
	<div id="left_panel">
		...
		<div id="file_tree" class="mytree hidden">
			<ul>'; $main_content .= !empty($nodes) ? AdminMenuUIHandler::getSubNodes($nodes, $main_layers_properties["expert"]) : ""; $main_content .= '
			</ul>
		</div>
	</div>
	<div id="hide_panel">
	...
	<div id="right_panel">'; $iframe_url = !empty($default_page) ? $default_page : $project_url_prefix . 'admin/admin_home'; $main_content .= '
		<iframe src="' . $iframe_url . '"></iframe>
		<div class="iframe_overlay">
			<div class="iframe_loading">Loading...</div>
		</div>
	</div>'; $main_content .= TourGuideUIHandler::getHtml($entity, $project_url_prefix, $project_common_url_prefix, $online_tutorials_url_prefix, array("restart_allow" => false)); } function getMainLayersProperties() { $v6f3a2700dd = '?admin_type=expert&path=#path#&action=#action#&extra=#extra#'; return array( "expert" => array( "ui" => array( "path" => "", "item_id" => "", "item_type" => "", "folder" => array( "get_sub_files_url" => "?admin_type=expert&path=#path#&action=get_sub_files", "attributes" => array( "rename_url" => $v6f3a2700dd, "remove_url" => $v6f3a2700dd, "create_url" => $v6f3a2700dd, "upload_url" => '?admin_type=expert&path=#path#&action=upload', "download_url" => '?admin_type=expert&path=#path#&action=download', "zip_url" => $v6f3a2700dd, "copy_url" => "[#path#]", "cut_url" => "[#path#]", "paste_url" => $v6f3a2700dd, ) ), "file" => array( "attributes" => array( "onClick" => "return goTo(this, 'edit_url', event)", "rename_url" => $v6f3a2700dd, "remove_url" => $v6f3a2700dd, "edit_url" => '?admin_type=expert&path=#path#&action=edit', "download_url" => '?admin_type=expert&path=#path#&action=download', "zip_url" => $v6f3a2700dd, "copy_url" => "[#path#]", "cut_url" => "[#path#]", ) ), "zip_file" => array( "attributes" => array( "rename_url" => $v6f3a2700dd, "remove_url" => $v6f3a2700dd, "download_url" => '?admin_type=expert&path=#path#&action=download', "unzip_url" => $v6f3a2700dd, "copy_url" => "[#path#]", "cut_url" => "[#path#]", ) ), ) ) ); } function getcontextMenusHtml() { return '
