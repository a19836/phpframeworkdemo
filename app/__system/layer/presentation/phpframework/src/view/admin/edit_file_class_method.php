...
var is_obj_valid = ' . ($is_obj_valid ? "true" : "false") . ';
	...
	'; $head .= WorkFlowPresentationHandler::getPresentationBrokersHtml($presentation_brokers, $choose_bean_layer_files_from_file_manager_url, $get_file_properties_url, $upload_bean_layer_files_from_file_manager_url); $head .= WorkFlowPresentationHandler::getBusinessLogicBrokersHtml($business_logic_brokers, $choose_bean_layer_files_from_file_manager_url, $get_file_properties_url); $head .= WorkFlowPresentationHandler::getDaoLibAndVendorBrokersHtml($choose_dao_files_from_file_manager_url, $choose_lib_files_from_file_manager_url, $choose_vendor_files_from_file_manager_url, $get_file_properties_url); $head .= WorkFlowPresentationHandler::getDataAccessBrokersHtml($data_access_brokers, $choose_bean_layer_files_from_file_manager_url); $head .= '</script>'; $main_content = '
		...
		</div>'; $main_content .= WorkFlowPresentationHandler::getChooseFromFileManagerPopupHtml($bean_name, $bean_file_name, $choose_bean_layer_files_from_file_manager_url, $choose_dao_files_from_file_manager_url, $choose_lib_files_from_file_manager_url, $choose_vendor_files_from_file_manager_url, $db_brokers, $data_access_brokers, $ibatis_brokers, $hibernate_brokers, $business_logic_brokers, $presentation_brokers); $main_content .= '<div id="choose_dao_object_from_file_manager" class="myfancypopup choose_from_file_manager with_title">
	...
	<div class="file_class_method_obj with_top_bar_tab">
		...
		<div id="settings" class="' . $ft . '_settings collapsed">
			...
			<div class="type">
				<label>Type:</label>
				<select>'; $types = array("public", "private", "protected"); $t = count($types); for ($i = 0; $i < $t; $i++) $main_content .= '<option' . (isset($obj_data["type"]) && $types[$i] == $obj_data["type"] ? " selected" : "") . '>' . $types[$i] . '</option>'; $main_content .= '
				</select>
			</div>
			<div class="abstract">
			...
			<div class="arguments">
				...
				<table>
					...
					<tbody class="fields">'; $arguments = isset($obj_data["arguments"]) ? $obj_data["arguments"] : null; if (is_array($arguments)) foreach ($arguments as $arg_name => $arg_value) $main_content .= WorkFlowPHPFileHandler::getArgumentHTML($arg_name, $arg_value); $main_content .= '
					</tbody>
				</table>
			</div>
		'; if (!empty($include_annotations)) { $main_content .= '
			<div class="annotations">
				...
				<table>
					...
					<tbody class="fields">'; if (isset($params) && is_array($params)) foreach ($params as $param) { $attrs = $param->getArgs(); if (!empty($obj_data["is_business_logic_service"]) && is_array($attrs) && isset($attrs["name"]) && substr($attrs["name"], 0, 5) == "data[") { $attrs["name"] = substr($attrs["name"], 5); $attrs["name"] = substr($attrs["name"], -1) == "]" ? substr($attrs["name"], 0, -1) : $attrs["name"]; $attrs["name"] = str_replace(array('"', "'"), "", $attrs["name"]); preg_match_all("/([^\[\]]+)/u", $attrs["name"], $matches, PREG_PATTERN_ORDER); $first = $matches ? array_shift($matches[1]) : ""; $attrs["name"] = $first . (!empty($matches[1]) ? "[" . implode('][', $matches[1]) . "]" : ""); } $main_content .= WorkFlowPHPFileHandler::getAnnotationHTML($attrs, "param"); } if (isset($returns) && is_array($returns)) foreach ($returns as $return) $main_content .= WorkFlowPHPFileHandler::getAnnotationHTML($return->getArgs(), "return"); $main_content .= '
					</tbody>
				</table>
			</div>'; } $main_content .= '
			<div class="comments">
	...
	<div class="big_white_panel"></div>'; $main_content .= TourGuideUIHandler::getHtml($entity, $project_url_prefix, $project_common_url_prefix, $online_tutorials_url_prefix); } else { $main_content = '
		<div class="top_bar' . ($popup ? " in_popup" : "") . '">
			<header>
