 ...
 */
include_once $EVC->getUtilPath("AdminMenuUIHandler"); if (empty($is_admin_ui_low_code_allowed)) { echo '<script>
...
</script>'; $main_content = AdminMenuUIHandler::getContextMenus($exists_db_drivers, $get_store_programs_url, $is_module_user_installed); if (empty($projects)) $main_content .= '<script>alert("Error: No projects available! Please contact your sysadmin...");</script>'; $main_content .= '
...
<div id="left_panel">
	<ul class="tabs">'; $available_layers = array("presentation_layers", "business_logic_layers", "data_access_layers", "db_layers"); if (!empty($layers)) { foreach ($available_layers as $layer_type_name) if (!empty($layers[$layer_type_name])) { $label = ucwords(str_replace("_", " ", $layer_type_name)); $main_content .= '<li class="tab tab_' . $layer_type_name . '"><a href="#' . $layer_type_name . '" title="' . $label . '"><i class="tab_icon main_node main_node_' . $layer_type_name . '"></i></a></li>'; } $main_content .= '<li class="tab tab_library"><a href="#library" title="Library"><i class="tab_icon main_node main_node_library"></i></a></li>'; } $main_content .= '
	</ul>'; if (!empty($layers)) { foreach ($available_layers as $layer_type_name) if (!empty($layers[$layer_type_name])) { $only_one_layer = count($layers[$layer_type_name]) == 1; $class = !$only_one_layer || $layer_type_name == "presentation_layers" ? "with_sub_groups" : ""; $main_content .= '<div id="' . $layer_type_name . '" class="layers ' . ($only_one_layer ? "with_sub_menus" : "without_sub_menus") . '"><div id="' . $layer_type_name . '_tree" class="mytree hidden ' . $class . '"><ul>'; foreach ($layers[$layer_type_name] as $layer_name => $layer) $main_content .= AdminMenuUIHandler::getLayer($layer_name, $layer, $main_layers_properties, $project_url_prefix, $filter_by_layout, $filter_by_layout_permission, isset($db_driver_broker_name) ? $db_driver_broker_name : null); $main_content .= '</ul></div>'; if ($only_one_layer) { $sub_menu_html = ""; if ($layer_type_name == "presentation_layers") $sub_menu_html = '
						<li class="level">
							<label>Level:</label>
			...
			<div id="library_tree" class="mytree hidden">
				<ul>'; $main_content .= isset($layers["libs"]["lib"]) ? AdminMenuUIHandler::getLayer("lib", $layers["libs"]["lib"], $main_layers_properties, $project_url_prefix) : ''; $main_content .= isset($layers["vendors"]["vendor"]) ? AdminMenuUIHandler::getLayer("vendor", $layers["vendors"]["vendor"], $main_layers_properties, $project_url_prefix) : ''; $main_content .= !empty($layers["others"]["other"]) ? AdminMenuUIHandler::getLayer("other", $layers["others"]["other"], $main_layers_properties, $project_url_prefix) : ''; $main_content .= '
				</ul>
			</div>
		</div>'; } $main_content .= '
</div>
