 ...
 */
include_once $EVC->getUtilPath("AdminMenuUIHandler"); include_once $EVC->getUtilPath("HeatMapHandler"); $head = AdminMenuUIHandler::getHeader($project_url_prefix, $project_common_url_prefix); $head .= '
<!-- Add Local JS and CSS files -->
<link rel="stylesheet" href="' . $project_url_prefix . 'css/admin/admin_uis.css" type="text/css" charset="utf-8" />'; if (empty($admin_uis_count)) { echo '<script>
	...
	</script>'; die(); } else if ($admin_uis_count == 1) { $default_admin_type = !empty($is_admin_ui_simple_allowed) ? "simple" : ( !empty($is_admin_ui_citizen_allowed) ? "citizen" : ( !empty($is_admin_ui_low_code_allowed) ? "low_code" : ( !empty($is_admin_ui_advanced_allowed) ? "advanced" : "expert" ) ) ); header("Location: {$project_url_prefix}admin?admin_type=" . $default_admin_type); } $head .= HeatMapHandler::getHtml($project_url_prefix); $main_content = '<div id="title">Please choose your workspace:</div>
<ul>
