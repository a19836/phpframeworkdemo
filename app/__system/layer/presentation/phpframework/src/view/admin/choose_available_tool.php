...
var is_popup = ' . ($popup ? 1 : 0) . ';
	...
	<ul>
		...
		<li class="doc_book" onClick="return goTo(\'' . $project_url_prefix . 'docbook/\', event)">
		...
		</li>'; if (!empty($layers)) { $filter_by_layout_url_query = $filter_by_layout ? "&filter_by_layout=$filter_by_layout&filter_by_layout_permission=$filter_by_layout_permission" : ""; if (isset($layers["vendors"]["vendor"])) $main_content .= '
			...
			</li>'; if (!empty($layers["others"]["other"])) $main_content .= '
		<li class="other_files" onClick="return goTo(\'' . $project_url_prefix . 'phpframework/presentation/list?item_type=other\', event)">
			...
			</li>'; $main_content .= '<li class="delimiter"></li>'; if (!empty($layers["db_layers"])) foreach ($layers["db_layers"] as $layer_name => $layer) { $bn = isset($layer["properties"]["bean_name"]) ? $layer["properties"]["bean_name"] : null; $bfn = isset($layer["properties"]["bean_file_name"]) ? $layer["properties"]["bean_file_name"] : null; $label = WorkFlowBeansFileHandler::getLayerBeanFolderName($user_beans_folder_path . $bfn, $bn, $user_global_variables_file_path); $main_content .= '
			...
			</li>'; } if (!empty($layers["data_access_layers"])) foreach ($layers["data_access_layers"] as $layer_name => $layer) { $bn = isset($layer["properties"]["bean_name"]) ? $layer["properties"]["bean_name"] : null; $bfn = isset($layer["properties"]["bean_file_name"]) ? $layer["properties"]["bean_file_name"] : null; $WorkFlowBeansFileHandler = new WorkFlowBeansFileHandler($user_beans_folder_path . $bfn, $user_global_variables_file_path); $obj = $WorkFlowBeansFileHandler->getBeanObject($bn); $obj_type = $obj->getType(); $label = WorkFlowBeansFileHandler::getLayerObjFolderName($obj); $main_content .= '
