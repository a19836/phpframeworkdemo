 ...
 */
if (empty($beans_xml_exception)) include $EVC->getViewPath($entity_view_id); else { echo '<html>
	<head>
	...
	<body>
		...
		<div id="bottom_panel">
			...
			Access the \'<a href="' . $project_url_prefix . 'setup?step=3.1&iframe=1&hide_setup=1" target="layers">Layers Setup</a>\'' . ( !empty($is_admin_ui_expert_allowed) ? ' 
				and \'<a href="' . $project_url_prefix . 'admin?admin_type=expert" target="expert">Expert Workspace</a>\'
			' : '' ) . ' to correct this issue.
		</div>'; launch_exception($beans_xml_exception); } ?>
