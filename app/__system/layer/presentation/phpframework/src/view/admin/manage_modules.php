...
'; $is_single_presentation_layer = count($modules) == 1; $main_content = '
...
<div class="modules_list' . ($popup ? ' in_popup' : '') . '">
	<div class="layer' . ($is_single_presentation_layer ? ' hidden' : '') . '">
		...
		<select onChange="showModulesLayer(this)" title="Choose a Presentation Layer">'; $t = count($modules); for ($i = 0; $i < $t; $i++) { $m = $modules[$i]; $is_selected = $default_presentation_layer_name && isset($m["bean_name"]) && $default_presentation_layer_name == $m["bean_name"]; $main_content .= '<option modules_id="layer_modules_' . $i . '"' . ($is_selected ? " selected" : "") . '>' . (isset($m["item_label"]) ? $m["item_label"] : "") . '</option>'; } $main_content .= '		
		</select>
	</div>'; for ($i = 0; $i < $t; $i++) { $m = $modules[$i]; $bean_name = isset($m["bean_name"]) ? $m["bean_name"] : null; $bean_file_name = isset($m["bean_file_name"]) ? $m["bean_file_name"] : null; $project_loaded_modules = isset($m["modules"]) ? $m["modules"] : null; $is_selected = $default_presentation_layer_name && $default_presentation_layer_name == $bean_name; $delete_module_url = $project_url_prefix . "phpframework/admin/manage_module?bean_name=$bean_name&bean_file_name=$bean_file_name&action=uninstall&module_id=#module_id#"; $disable_module_url = $project_url_prefix . "phpframework/admin/manage_module?bean_name=$bean_name&bean_file_name=$bean_file_name&action=disable&module_id=#module_id#"; $enable_module_url = $project_url_prefix . "phpframework/admin/manage_module?bean_name=$bean_name&bean_file_name=$bean_file_name&action=enable&module_id=#module_id#"; $main_content .= '<div id="layer_modules_' . $i . '" class="layer_modules">'; if ($is_install_module_allowed) $main_content .= '
	<div class="install">
	...
	<table>
		...
		<tbody>'; if (is_array($loaded_modules)) { foreach ($loaded_modules as $group_module_id => $loaded_modules_by_group) { $sub_main_content = ""; foreach ($loaded_modules_by_group as $module_id => $loaded_module) if (!empty($project_loaded_modules[$module_id])) { $enable = isset($project_loaded_modules[$module_id]["path"]) && CMSModuleEnableHandler::isModuleEnabled($project_loaded_modules[$module_id]["path"]); $admin_url = !empty($loaded_module["admin_path"]) ? $project_url_prefix . "phpframework/admin/module_admin?bean_name=$bean_name&bean_file_name=$bean_file_name" . ($is_selected ? '&filter_by_layout=' . $filter_by_layout : '') . "&group_module_id=$group_module_id" . ($popup ? "&popup=$popup" : '') : null; $image = ''; if (!empty($loaded_module["images"][0]["url"])) { if (preg_match("/\.svg$/i", $loaded_module["images"][0]["url"]) && !empty($loaded_module["images"][0]["path"]) && file_exists($loaded_module["images"][0]["path"])) $image = file_get_contents($loaded_module["images"][0]["path"]); else $image = '<img src="' . $loaded_module["images"][0]["url"] . '" />'; } $loaded_module_id = isset($loaded_module["id"]) ? $loaded_module["id"] : null; $sub_main_content .= '<tr class="group_module_item" group_module_id="' . $group_module_id . '">
						<td class="group"></td>
