...
?>'; if (file_put_contents($user_global_settings_file_path, $content)) { $status_message = "Settings saved successfully"; } else { $error_message = "There was an error trying to save settings. Please try again..."; } } $vars = PHPVariablesFileHandler::getVarsFromFileContent($user_global_settings_file_path); $vars["default_timezone"] = !empty($vars["default_timezone"]) ? $vars["default_timezone"] : (isset($GLOBALS["default_timezone"]) ? $GLOBALS["default_timezone"] : null); $vars["die_when_throw_exception"] = !empty($vars["die_when_throw_exception"]) ? $vars["die_when_throw_exception"] : (!empty($GLOBALS["die_when_throw_exception"]) ? "true" : "false"); $vars["log_level"] = isset($vars["log_level"]) && is_numeric($vars["log_level"]) ? $vars["log_level"] : (isset($GLOBALS["log_level"]) ? $GLOBALS["log_level"] : null); $vars["log_echo_active"] = !empty($vars["log_echo_active"]) ? $vars["log_echo_active"] : (!empty($GLOBALS["log_echo_active"]) ? "true" : "false"); $vars["log_file_path"] = !empty($vars["log_file_path"]) ? $vars["log_file_path"] : (isset($GLOBALS["log_file_path"]) ? $GLOBALS["log_file_path"] : null); $vars["tmp_path"] = !empty($vars["tmp_path"]) ? $vars["tmp_path"] : (isset($GLOBALS["tmp_path"]) ? $GLOBALS["tmp_path"] : null); ?>
