	...
	return {
		...
		setup: function(editor) {
			...
			editor.on('change', function(e) {
				...
				if (win.tinymce_editor_on_change_timeout_id)
					clearTimeout(win.tinymce_editor_on_change_timeout_id);
				
				win.tinymce_editor_on_change_timeout_id = setTimeout(function() {
...
function cleanTinyMCEInstances(region_blocks) {
...
function loadAvailableBlocksList(parent, opts) {
	parent = parent instanceof jQuery ? parent : $(parent);
	//console.log("loadAvailableBlocksList");
...
function loadAvailableBlocksListInRegionsBlocks(parent, opts) {
	...
	else if (opts && typeof opts["on_load_available_blocks_handler"] == "function")
		opts["on_load_available_blocks_handler"](parent);
}
...
function updateSelectedTemplateRegionsBlocks(p, data) {
	...
	cleanTinyMCEInstances(region_blocks); //clean tinymce instances before it empty the html from region_blocks. This is very important, otherwise the tinymce is overloading the browser memory with too many instances...
...
function updateLayoutIframeFromSettings(iframe, data, iframe_html_to_parse) {
	if (iframe_html_to_parse)
		...
		if (!template_regions) {
			cleanTinyMCEInstances(iframe_body); //clean tinymce instances before it empty the html from region_blocks. This is very important, otherwise the tinymce is overloading the browser memory with too many instances...
			
			iframe_body.find(".template_region > .items").html("");
		}
		else {
			var iframe_data = iframe[0].contentWindow.getTemplateRegionsBlocks();
			...
			if (iframe_data["regions_blocks"])
				$.each(iframe_data["regions_blocks"], function(idx, region_item) {
			...
			$.each(template_regions, function(region_name, region_items) {
				...
				var template_region_items = template_region_elm.children(".items");
				
				//if template_region does not exists in iframe, reload iframe and get out from loop
				if (template_region_items.length == 0) {
					reloadLayoutIframeFromSettings(iframe, data);
					return false;
				}
				else if (!$.isArray(region_items) && !$.isPlainObject(region_items) && iframe_region_items && iframe_region_items.length > 0) { //if region has no items, set correspondent iframe .items with empty html.
					cleanTinyMCEInstances(template_region_items); //clean tinymce instances before it empty the html from region_blocks. This is very important, otherwise the tinymce is overloading the browser memory with too many instances...
						
					template_region_items.html("");
				}
				else {
					//removes all children that are not .item
					...
					var iframe_region_items_children = template_region_items.children(".item");
					...
					$.each(region_items, function(idx, region_item) {
						...
						var iframe_item = $(iframe_region_items_children[item_to_insert_index]); //get the iframe_item where the items wil be inserted before...
						var is_html = region_item[3];
						
						if (!is_html)
							item_to_insert_index++; //prepare the next index for iframe_item
						
						if (!iframe_region_item || JSON.stringify(region_item) != JSON.stringify(iframe_region_item)) {
							...
							if (iframe_item[0]) {
								...
								if (!is_html) {
									...
									cleanTinyMCEInstances(iframe_item); //clean tinymce instances before it empty the html from region_blocks. This is very important, otherwise the tinymce is overloading the browser memory with too many instances...
									iframe_item.remove();
								}
							}
							else //insert new item at the end of .items
...
function initPageAndTemplateLayout(main_parent_elm, main_layout_elm, on_ready_callback) {
	...
	if (iframe[0]) {
		...
		iframe.load(function() {
			...
			loadAvailableBlocksList(regions_blocks_includes_settings, {
				...
				"on_load_available_blocks_handler" : function() {
					...
					if (typeof on_ready_callback == "function") //must be inside here bc of the update_settings_from_layout_iframe_func and update_layout_iframe_from_settings_func vars, otherwise the loadAvailableBlocksList will trigger some select.change events that will trigger this functions
						on_ready_callback();
				},
			});
		});
		...
		if (iframe.parent().is(".tab_content_template_layout"))
	      	...
	      	setTimeout(function() {
				...
				var region = droppable.attr("region");
			 	...
			 	if (!new_item) {
			    		...
			    		//last_item.children(".icon.add").trigger("click"); //add new region-block item
			    		droppable.find(" > .template_region_name > .template_region_name_link").trigger("click"); //add new region-block item
