...
if (!empty($PEVC)) {
	...
	if (!empty($_POST)) {
		if (!empty($_POST["add"]) || !empty($_POST["save"])) {
			...
			$status = !empty($_POST["add"]) ? ZipUtil::insertState($brokers, $data) : ZipUtil::updateState($brokers, $data);
		}
	...
	//Preparing HTML
	$form_settings = array(
		"title" => $data || (!empty($_POST["delete"]) && empty($error_message)) ? "Edit State '$state_id'" : "Add State",
		"fields" => array(
