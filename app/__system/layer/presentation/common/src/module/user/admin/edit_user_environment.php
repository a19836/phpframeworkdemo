...
if (!empty($PEVC)) {
		...
		if (!empty($action)) {
			if (!empty($status)) {
				...
				if (!empty($_POST["add"])) {
					...
					$url = $CommonModuleAdminUtil->getAdminFileUrl("edit_user_environment") . "user_id=$data_user_id&environment_id=$data_environment_id";
	...
	//Preparing HTML
	$form_settings = array(
		"title" => $data || (!empty($_POST["delete"]) && empty($error_message)) ? "Edit User Environment" : "Add User Environment",
		"fields" => array(
