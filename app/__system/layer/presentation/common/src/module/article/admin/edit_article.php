...
if (!empty($PEVC)) {
	...
	if (!empty($_POST)) {
		if (!empty($_POST["save"]) || !empty($_POST["add"])) {
			...
			$status = ArticleUtil::setArticleProperties($PEVC, $article_id, $data, isset($_FILES["photo"]) ? $_FILES["photo"] : null);
	...
	//Preparing HTML
	$form_settings = array(
		"title" => $data || (!empty($_POST["delete"]) && empty($error_message)) ? "Edit Article '$article_id'" : "Add Article",
		"fields" => array(
			"title" => "text",
			"sub_title" => "text",
			"published" => array(
