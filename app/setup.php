...
function checkFilesPermission($files, $optional_files, $check_folder_sub_files, &$main_status) { $file_statuses = array(); foreach ($files as $file) { $optional = in_array($file, $optional_files); $exists = file_exists($file); if ($exists || !$optional) { $is_writable = $exists ? is_writable($file) : false; $file_statuses[$file] = array($is_writable, null); if (!$is_writable) $main_status = false; } } foreach ($file_statuses as $file => $file_props) { $is_writable = $file_props[0]; if ($is_writable) { $check_sub_files = in_array($file, $check_folder_sub_files); if ($check_sub_files) { $incorrect_sub_files = checkSubFilesPermission($file, $optional_files, $main_status); if (count($incorrect_sub_files) > 0) $file_statuses[$file][1] = $incorrect_sub_files; } } } $html = "<ul>"; foreach ($file_statuses as $file => $file_props) { $is_writable = $file_props[0]; $incorrect_sub_files = $file_props[1]; $path = $exists && !empty(realpath($file)) ? realpath($file) : $file; $html .= "<li>" . $path . ": " . printWritableStatus($is_writable); if ($incorrect_sub_files) { $html .= "<ul>"; foreach ($incorrect_sub_files as $sub_file) if (!in_array($sub_file, $files)) $html .= "<li>" . $sub_file . ": " . printWritableStatus(false) . "</li>"; $html .= "</ul>"; } $html .= "</li>\n"; } $html .= "</ul>"; return $html; } function checkSubFilesPermission($file, $optional_files, &$main_status) { return array(); $incorrect_files = array(); if (is_dir($file)) { $file .= substr($file, -1) != "/" ? "/" : ""; $sub_files = array_diff(scandir($file), array("..", ".", ".gitkeep", ".htaccess", ".htpasswd", ".git")); if ($sub_files) { foreach ($sub_files as $i => $sub_file) { $sub_file = $file . $sub_file; $optional = in_array($sub_file, $optional_files); if (!$optional) { $is_writable = is_writable($sub_file); if (!$is_writable) { $incorrect_files[] = $sub_file; $main_status = false; } else if (is_dir($sub_file)) { $incorrect_sub_files = checkSubFilesPermission($sub_file, $optional_files, $main_status); if (count($incorrect_sub_files) > 0) $incorrect_files = array_merge($incorrect_files, $incorrect_sub_files); } } } } } return $incorrect_files; } function printWritableStatus($status) { return $status ? '<span class="writeable">OK</span>' : '<span class="non_writeable">NON WRITEABLE</span>'; } function printOptionalStatus($status) { return $status ? '<span class="looks_ok">LOOKS OK</span>' : '<span class="looks_non_ok">PLEASE CHECK</span>'; } $dir_path = str_replace(DIRECTORY_SEPARATOR, "/", __DIR__) . "/"; $installation_dir = dirname($dir_path) . "/"; $main_status = true; $script_name = isset($_SERVER["SCRIPT_NAME"]) ? $_SERVER["SCRIPT_NAME"] : null; $local_installation_name = strstr($script_name, "/" . basename(__DIR__) . "/", true); $document_root = (!empty($_SERVER["CONTEXT_DOCUMENT_ROOT"]) ? $_SERVER["CONTEXT_DOCUMENT_ROOT"] : (isset($_SERVER["DOCUMENT_ROOT"]) ? $_SERVER["DOCUMENT_ROOT"] : null) ) . "/"; $document_root = preg_replace("/[\/]+/", "/", $document_root); if ($local_installation_name && is_dir($document_root . $local_installation_name . "/tmp/")) $tmp_path = $document_root . $local_installation_name . "/tmp/"; else if (is_dir($document_root . "/tmp/")) $tmp_path = $document_root . "/tmp/"; else $tmp_path = (sys_get_temp_dir() ? sys_get_temp_dir() : "/tmp") . "/phpframework/"; $tmp_path = preg_replace("/\/\/+/", "/", $tmp_path); @mkdir($tmp_path, 0755, true); $files = array( $tmp_path, $installation_dir . "files/", $installation_dir . "vendor/", $installation_dir . "vendor/dao/", $installation_dir . "vendor/codeworkfloweditor/", $installation_dir . "vendor/codeworkfloweditor/task/", $installation_dir . "vendor/layoutuieditor/", $installation_dir . "vendor/layoutuieditor/widget/", $installation_dir . "vendor/testunit/", $installation_dir . "other/authdb/", $installation_dir . "other/authdb/permission.tbl", $installation_dir . "other/authdb/user.tbl", $installation_dir . "other/authdb/user_type.tbl", $installation_dir . "other/authdb/user_type_permission.tbl", $installation_dir . "other/authdb/user_stats.tbl", $installation_dir . "other/authdb/user_user_type.tbl", $installation_dir . "other/authdb/login_control.tbl", $installation_dir . "other/authdb/layout_type.tbl", $installation_dir . "other/authdb/layout_type_permission.tbl", $installation_dir . "other/authdb/module_db_table_name.tbl", $installation_dir . "other/authdb/object_type.tbl", $installation_dir . "other/authdb/reserved_db_table_name.tbl", $installation_dir . "other/workflow/", $installation_dir . "app/config/", $installation_dir . "app/layer/", $installation_dir . "app/layer/.htaccess", $installation_dir . "app/lib/vendor/", $installation_dir . "app/__system/config/global_settings.php", $installation_dir . "app/__system/config/global_variables.php", $installation_dir . "app/__system/layer/presentation/phpframework/src/config/authentication.php", $installation_dir . "app/__system/layer/presentation/phpframework/webroot/vendor/", $installation_dir . "app/__system/layer/presentation/phpframework/webroot/__system/", $installation_dir . "app/__system/layer/presentation/test/webroot/__system/", $installation_dir . "app/__system/layer/presentation/common/webroot/cms/", $installation_dir . "app/__system/layer/presentation/common/webroot/__system/", $installation_dir . "app/__system/layer/presentation/common/src/module/", $installation_dir . "app/__system/layer/presentation/common/webroot/module/", $installation_dir . "app/__system/layer/presentation/common/webroot/vendor/", $installation_dir . "app/__system/layer/presentation/common/webroot/vendor/ckeditor/", $installation_dir . "app/__system/layer/presentation/common/webroot/vendor/tinymce/", $installation_dir . "app/lib/vendor/phpjavascriptpacker/", $installation_dir . "app/lib/vendor/phpmailer/", $installation_dir . "app/lib/vendor/xsssanitizer/", ); $optional_files = array( $installation_dir . "files/", $installation_dir . "other/authdb/login_control.tbl", $installation_dir . "other/authdb/user_stats.tbl", $installation_dir . "other/authdb/layout_type.tbl", $installation_dir . "other/authdb/layout_type_permission.tbl", $installation_dir . "app/__system/layer/presentation/test/webroot/__system/", $installation_dir . "app/__system/layer/presentation/common/webroot/__system/", $installation_dir . "app/__system/layer/presentation/common/webroot/vendor/ckeditor/", $installation_dir . "app/__system/layer/presentation/common/webroot/vendor/tinymce/", $installation_dir . "app/lib/vendor/phpjavascriptpacker/", $installation_dir . "app/lib/vendor/phpmailer/", $installation_dir . "app/lib/vendor/xsssanitizer/", ); $check_folder_sub_files = array( $tmp_path, $installation_dir . "files/", $installation_dir . "vendor/", $installation_dir . "other/authdb/", $installation_dir . "other/workflow/", $installation_dir . "app/config/", $installation_dir . "app/layer/", $installation_dir . "app/__system/layer/presentation/phpframework/webroot/__system/", $installation_dir . "app/__system/layer/presentation/test/webroot/__system/", $installation_dir . "app/__system/layer/presentation/common/webroot/cms/", $installation_dir . "app/__system/layer/presentation/common/webroot/__system/", $installation_dir . "app/__system/layer/presentation/common/src/module/", $installation_dir . "app/__system/layer/presentation/common/webroot/module/", $installation_dir . "app/__system/layer/presentation/common/webroot/vendor/ckeditor/", $installation_dir . "app/__system/layer/presentation/common/webroot/vendor/tinymce/", $installation_dir . "app/lib/vendor/phpjavascriptpacker/", $installation_dir . "app/lib/vendor/phpmailer/", $installation_dir . "app/lib/vendor/xsssanitizer/", ); if ($tmp_path != $installation_dir . "tmp/") { array_unshift($files, $installation_dir . "tmp/"); $optional_files[] = $installation_dir . "tmp/"; $check_folder_sub_files[] = $installation_dir . "tmp/"; } if ($tmp_path != $installation_dir . "app/tmp/") { array_unshift($files, $installation_dir . "app/tmp/"); $optional_files[] = $installation_dir . "app/tmp/"; $check_folder_sub_files[] = $installation_dir . "app/tmp/"; } $setup_already_executed = is_dir($installation_dir . "app/layer/") && count(array_diff(scandir($installation_dir . "app/layer/"), array("..", ".", ".gitkeep", ".htaccess", ".htpasswd", ".git"))) > 0; $document_root_status = $installation_dir == $document_root; $php_version_status = version_compare(PHP_VERSION, '5.6', '>='); $loaded_extensions = array_map("strtolower", get_loaded_extensions()); $current_user_info = function_exists("posix_getpwuid") ? posix_getpwuid(posix_getuid()) : null; $is_apache_user = isset($current_user_info["name"]) && $current_user_info["name"] == "www-data"; $parsedown_path = $installation_dir . "app/lib/vendor/parsedown/Parsedown.php"; $md_contents = file_get_contents($installation_dir . "INSTALL.md"); if (file_exists($parsedown_path)) { include_once $parsedown_path; $Parsedown = new Parsedown(); $inner_html = $Parsedown->text($md_contents); $inner_html = str_replace("<p>.</p>", "<p>&nbsp;</p>", $inner_html); } else { $inner_html = '<pre>' . str_replace("<br/>", "\n", str_replace("\n.\n", "\n\n", $md_contents)) . '</pre>'; } $html = ""; if ($setup_already_executed) $html .= '<div class="setup_already_executed">It seems this setup was already executed.<br/>If you continue, it may overwrite some of your changes...</div>'; $html .= "<ol>
	...
	<div class=\"button\"><button onClick=\"document.location='__system/setup/'\"" . ($main_status ? "" : 'disabled title="Something above is not OK"') . ">Proceed with Setup</button></div>"; echo $html; ?>
