...
class CountryService extends \soa\CommonService {
	...
	public function insertCountry($data) {
		...
		if (is_a($b, "IIbatisDataAccessBrokerClient")) {
			...
			return $status ? (!empty($data["country_id"]) ? $data["country_id"] : $b->getInsertedId($options)) : $status;
		}
	...
	public function deleteCountriesByConditions($data) {
		...
		if ($conditions) {
			...
			if (is_a($b, "IIbatisDataAccessBrokerClient")) {
				...
				$Country = $this->getCountryHbnObj($b, $options);
				return $Country->deleteByConditions(array("conditions" => $conditions, "conditions_join" => $conditions_join), $options);
			}
			else if (is_a($b, "IDBBrokerClient")) {
	...
	public function countCountriesByConditions($data) {
		...
		if ($conditions) {
			...
			if (is_a($b, "IIbatisDataAccessBrokerClient")) {
				...
				$Country = $this->getCountryHbnObj($b, $options);
				return $Country->count(array("conditions" => $conditions, "conditions_join" => $conditions_join), $options);
			}
			else if (is_a($b, "IDBBrokerClient")) {
				$options = $options ? $options : array();
				$options["conditions_join"] = $conditions_join;
