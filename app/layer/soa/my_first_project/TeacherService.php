...
class TeacherService extends \soa\CommonService {
	 ...
	 * @param (name=data[age], type=org.phpframework.object.db.DBPrimitive(tinyint), not_null=1, length=2)  
	 */
	public function insert($data) {
		$options = $data["options"];
		$this->mergeOptionsWithBusinessLogicLayer($options);
		
		if (isset($data["teacher_id"]) && is_string($data["teacher_id"]) && is_numeric($data["teacher_id"])) $data["teacher_id"] += 0;
			if (isset($data["school_id"]) && is_string($data["school_id"]) && is_numeric($data["school_id"])) $data["school_id"] += 0;
			if (isset($data["age"]) && is_string($data["age"]) && is_numeric($data["age"])) $data["age"] += 0;
	...
	public function updatePrimaryKeys($data) {
		...
		if (isset($data["new_teacher_id"]) && is_string($data["new_teacher_id"]) && is_numeric($data["new_teacher_id"])) $data["new_teacher_id"] += 0;
			if (isset($data["old_teacher_id"]) && is_string($data["old_teacher_id"]) && is_numeric($data["old_teacher_id"])) $data["old_teacher_id"] += 0;
	 ...
	 * @param (name=data[name], type=org.phpframework.object.db.DBPrimitive(varchar), not_null=1, default="", length=50, add_sql_slashes=1, sanitize_html=1)  
	 * @param (name=data[age], type=org.phpframework.object.db.DBPrimitive(tinyint), not_null=1, length=2)
	 * @param (name=data[conditions][teacher_id], type=array|org.phpframework.object.db.DBPrimitive(bigint), length=20)  
	 ...
	 * @param (name=data[conditions][age], type=array|org.phpframework.object.db.DBPrimitive(tinyint), length=2)  
	 */
