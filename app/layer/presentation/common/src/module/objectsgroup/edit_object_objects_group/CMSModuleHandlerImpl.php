	...
	public function execute(&$settings = false) {
		...
		if (!empty($_POST)) {
			if (!empty($_POST["delete"]) && !empty($settings["allow_deletion"])) {
				$data_objects_group_id = isset($data["objects_group_id"]) ? $data["objects_group_id"] : null;
				...
				$status = !$data || \ObjectsGroupUtil::deleteObjectObjectsGroup($brokers, $data_objects_group_id, $data_object_type_id, $data_object_id);
			}
			else if (!empty($_POST["save"])) {
				$new_objects_group_id = isset($_POST["objects_group_id"]) ? $_POST["objects_group_id"] : null;
				...
				if ($empty_field_name) {
					...
					if (!empty($settings["allow_insertion"]) && empty($data)) {
						...
						if (\CommonModuleUI::areFieldsValid($EVC, $settings, $new_data, $error_message)) {
							$new_data["object_objects_groups"] = isset($settings["object_to_objects"]) ? $settings["object_to_objects"] : null;
							...
							if (isset($settings["on_insert_ok_action"]) && strpos($settings["on_insert_ok_action"], "_redirect") !== false) {
								$settings["on_insert_ok_redirect_url"] .= (isset($settings["on_insert_ok_redirect_url"]) && strpos($settings["on_insert_ok_redirect_url"], "?") !== false ? "&" : "?") . "objects_group_id=${new_data['objects_group_id']}&object_type_id=${new_data['object_type_id']}&object_id=${new_data['object_id']}";
							}
						}
					}
					else if (!empty($settings["allow_update"]) && $data) {
						$new_data = array();
						$new_data["old_objects_group_id"] = isset($data["objects_group_id"]) ? $data["objects_group_id"] : null;
						...
						$new_data["new_objects_group_id"] = !empty($settings["show_objects_group_id"]) ? $new_objects_group_id : $new_data["old_objects_group_id"];
						...
						if (\CommonModuleUI::areFieldsValid($EVC, $settings, $fields_to_validade, $error_message)) {
							...
							if (isset($settings["on_update_ok_action"]) && strpos($settings["on_update_ok_action"], "_redirect") !== false) {
								$settings["on_update_ok_redirect_url"] .= (isset($settings["on_update_ok_redirect_url"]) && strpos($settings["on_update_ok_redirect_url"], "?") !== false ? "&" : "?") . "objects_group_id=${new_data['new_objects_group_id']}&object_type_id=${new_data['new_object_type_id']}&object_id=${new_data['new_object_id']}";
							}
						}
					}
				}
		...
		if (!empty($_POST["save"])) {
			$form_data = array(
				"objects_group_id" => !empty($settings["show_objects_group_id"]) ? $new_objects_group_id : (isset($data["objects_group_id"]) ? $data["objects_group_id"] : null),
