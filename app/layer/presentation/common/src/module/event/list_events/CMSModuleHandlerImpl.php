	...
	public function execute(&$settings = false) {
		...
		if ($events) {
			...
			foreach ($events as &$event) {
				$event["begin_date"] = !empty($event["begin_date"]) ? substr($event["begin_date"], 0, strrpos($event["begin_date"], ":")) : '';
				$event["begin_date"] = $event["begin_date"] == '0000-00-00 00:00' ? '' : $event["begin_date"];
				
				$event["end_date"] = !empty($event["end_date"]) ? substr($event["end_date"], 0, strrpos($event["end_date"], ":")) : '';
		...
		$html = '<div class="module_list_events ' . (isset($settings["block_class"]) ? $settings["block_class"] : null) . '">';
		...
		$settings["class"] = "";
		$settings["edit_page_url"] .= (isset($settings["edit_page_url"]) && strpos($settings["edit_page_url"], "?") !== false ? "&" : "?") . "event_id=#[idx][event_id]#";
