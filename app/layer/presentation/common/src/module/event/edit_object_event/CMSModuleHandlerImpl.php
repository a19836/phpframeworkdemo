...
class CMSModuleHandlerImpl extends \CMSModuleHandler {
	...
	public function execute(&$settings = false) {
		...
		$object_type_id = isset($_GET["object_type_id"]) ? $_GET["event_id"] : null;
		$object_id = isset($_GET["object_id"]) ? $_GET["event_id"] : null;
		...
		if (!empty($_POST)) {
			if (!empty($_POST["delete"]) && !empty($settings["allow_deletion"])) {
				...
				$status = !$data || \EventUtil::deleteObjectEvent($brokers, $data_event_id, $data_object_type_id, $data_object_id);
			}
			else if (!empty($_POST["save"])) {
				$new_event_id = isset($_POST["event_id"]) ? $_POST["event_id"] : null;
					...
					if (!empty($settings["allow_insertion"]) && empty($data)) {
						...
						if (\CommonModuleUI::areFieldsValid($EVC, $settings, $new_data, $error_message)) {
							...
							if (isset($settings["on_insert_ok_action"]) && strpos($settings["on_insert_ok_action"], "_redirect") !== false) {
								$settings["on_insert_ok_redirect_url"] .= (isset($settings["on_insert_ok_redirect_url"]) && strpos($settings["on_insert_ok_redirect_url"], "?") !== false ? "&" : "?") . "event_id=${new_data['event_id']}&object_type_id=${new_data['object_type_id']}&object_id=${new_data['object_id']}";
							}
						}
					}
					else if (!empty($settings["allow_update"]) && $data) {
						$new_data = array();
						$new_data["old_event_id"] = isset($data["event_id"]) ? $data["event_id"] : null;
						...
						$new_data["new_event_id"] = !empty($settings["show_event_id"]) ? $new_event_id : $new_data["old_event_id"];
						...
						if (\CommonModuleUI::areFieldsValid($EVC, $settings, $fields_to_validade, $error_message)) {
							...
							if (isset($settings["on_update_ok_action"]) && strpos($settings["on_update_ok_action"], "_redirect") !== false) {
								$settings["on_update_ok_redirect_url"] .= (isset($settings["on_update_ok_redirect_url"]) && strpos($settings["on_update_ok_redirect_url"], "?") !== false ? "&" : "?") . "event_id=${new_data['new_event_id']}&object_type_id=${new_data['new_object_type_id']}&object_id=${new_data['new_object_id']}";
							}
						}
					}
				}
		...
		if (!empty($_POST["save"])) {
			$form_data = array(
				"event_id" => !empty($settings["show_"]) ? $new_event_id : (isset($data["event_id"]) ? $data["event_id"] : null),
