...
class CMSModuleHandlerImpl extends \CMSModuleHandler {
	...
	public function execute(&$settings = false) {
		...
		$username_attribute_label = isset($settings["fields"]["username"]["field"]["label"]["value"]) ? $settings["fields"]["username"]["field"]["label"]["value"] : null;
		$password_attribute_label = isset($settings["fields"]["password"]["field"]["label"]["value"]) ? $settings["fields"]["password"]["field"]["label"]["value"] : null;
		...
		$username_attribute_default_value = isset($settings["username_default_value"]) ? $settings["username_default_value"] : null;
		$password_attribute_default_value = isset($settings["password_default_value"]) ? $settings["password_default_value"] : null;
		...
		$show_captcha = !empty($settings["show_captcha"]) && !empty($_COOKIE[$show_captcha_var_name]) ? true : false;
		...
		if (!empty($_POST["login"])) {
			...
			if ($empty_field_name)
				...
				if ($user_session == \UserUtil::USER_BLOCKED) {
					...
					if (!empty($user_session["logged_status"])) {
						$show_captcha = false;
						$error_message = "Login successfully...";
					}
					else {
						if (!empty($settings["show_captcha"]) && isset($user_session["failed_login_attempts"]) && isset($settings["maximum_login_attempts_to_show_captcha"]) && $user_session["failed_login_attempts"] >= $settings["maximum_login_attempts_to_show_captcha"])
							$show_captcha = true;
		...
		if ($session_id && isset($user_session) && is_array($user_session) && !empty($user_session["session_id"]) && $session_id != $user_session["session_id"])
		...
		$session_id = !empty($user_session["session_id"]) ? $user_session["session_id"] : $session_id;
		$show_captcha = !empty($settings["show_captcha"]) ? $show_captcha : false;
		...
		//Preparing HTML
		$html .= '<div class="module_login ' . (isset($settings["block_class"]) ? $settings["block_class"] : "") . '">';
		
		if  (!empty($user_session["logged_status"])) {
			if (!empty($settings["welcoming_message"])) {
			...
			$settings["fields"]["username"]["field"]["class"] = "form_field " . (isset($settings["fields"]["username"]["field"]["class"]) ? $settings["fields"]["username"]["field"]["class"] : "");
			...
			$settings["fields"]["password"]["field"]["class"] = "form_field " . (isset($settings["fields"]["password"]["field"]["class"]) ? $settings["fields"]["password"]["field"]["class"] : null);
	...
	<span class="captcha_refresh">
		<a href="#" onclick="$(this).parent().parent().find(\'.captcha_image img\')[0].src=\'' . $captcha_url . '&\'+Math.random(); $(this).parent().parent().find(\'.form_field input\').focus();" id="change-image">' . translateProjectText($EVC, "Not readable? Change text.") . '</a>
			...
			if (!empty($settings["register_page_url"]))
			...
			if (!empty($settings["forgot_credentials_page_url"]))
			...
			if (!empty($settings["single_sign_on_page_url"]))
			...
			$form_data = array(
				"username" => !empty($username) ? $username : $username_attribute_default_value,
				"password" => !empty($password) ? $password : $password_attribute_default_value,
			);
			...
			if (!empty($settings["ptl"])) {
				...
				$form_settings["form_containers"][0]["container"]["elements"] = isset($settings["fields"]) ? $settings["fields"] : null;
