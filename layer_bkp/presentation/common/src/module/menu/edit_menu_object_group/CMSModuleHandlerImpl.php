	...
	public function execute(&$settings = false) {
		...
		if ($_POST) {
			if ($_POST["delete"] && $settings["allow_deletion"]) {
				$status = !$data || \MenuUtil::deleteMenuObjectGroup($brokers, $data["group_id"], $data["object_type_id"], $data["object_id"]);
			}
			else if ($_POST["save"]) {
				$new_group_id = $_POST["group_id"];
					...
					if ($settings["allow_insertion"] && empty($data)) {
						...
						if (\CommonModuleUI::areFieldsValid($EVC, $settings, $new_data, $error_message)) {
							...
							if (strpos($settings["on_insert_ok_action"], "_redirect") !== false) {
								$settings["on_insert_ok_redirect_url"] .= (strpos($settings["on_insert_ok_redirect_url"], "?") !== false ? "&" : "?") . "group_id=${new_data['group_id']}&object_type_id=${new_data['object_type_id']}&object_id=${new_data['object_id']}";
							}
						}
					}
					else if ($settings["allow_update"] && $data) {
						$new_data = array();
						$new_data["old_group_id"] = $data["group_id"];
						$new_data["old_object_type_id"] = $data["object_type_id"];
						$new_data["old_object_id"] = $data["object_id"];
						$new_data["new_group_id"] = $settings["show_group_id"] ? $new_group_id : $data["group_id"];
						...
						if (\CommonModuleUI::areFieldsValid($EVC, $settings, $fields_to_validade, $error_message)) {
							...
							if (strpos($settings["on_update_ok_action"], "_redirect") !== false) {
								$settings["on_update_ok_redirect_url"] .= (strpos($settings["on_update_ok_redirect_url"], "?") !== false ? "&" : "?") . "group_id=${new_data['new_group_id']}&object_type_id=${new_data['new_object_type_id']}&object_id=${new_data['new_object_id']}";
							}
						}
					}
				}
		...
		if ($_POST["save"]) {
			$form_data = array(
				"group_id" => $settings["show_group_id"] ? $new_group_id : $data["group_id"],
